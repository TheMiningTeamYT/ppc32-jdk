...
openjssever=OpenJSSE_1_1_4

if ! [ -e openjsse ]; then
	git clone https://gitlab.azulsystems.com/openjsse/openjsse.git -b $openjssever
else
	(
		git -C openjsse fetch;
                git -C openjsse checkout $openjssever
	)
fi

legacy8ujssever=Legacy8uJSSE_1_1_0

if ! [ -e legacy8ujsse ]; then
	git clone https://gitlab.azulsystems.com/openjsse/legacy8ujsse.git -b $legacy8ujssever
else
	(
		git -C legacy8ujsse fetch;
		git -C legacy8ujsse checkout $legacy8ujssever
	)
fi
