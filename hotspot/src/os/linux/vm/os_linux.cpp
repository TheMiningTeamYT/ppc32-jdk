...
void * os::dll_load(const char *filename, char *ebuf, int ebuflen)
{
  ...
  static const arch_t arch_array[]={
    ...
    #error Method os::dll_load requires that one of following is defined:\
      IA32, AMD64, IA64, __sparc, __powerpc__, ARM, S390, ALPHA, MIPS, MIPSEL, __mips64, PARISC, M68K, AARCH64, AARCH32
  #endif
...
size_t os::Linux::find_large_page_size() {
  ...
  large_page_size = IA32_ONLY(4 * M) AMD64_ONLY(2 * M) IA64_ONLY(256 * M) SPARC_ONLY(4 * M)
                     ARM_ONLY(2 * M) PPC_ONLY(4 * M) AARCH64_ONLY(2 * M) AARCH32_ONLY(2 * M)
                     MIPS64_ONLY(4 * M) MIPS32_ONLY(2 * M); //In MIPS _large_page_size is seted 4*M.
#endif // ZERO
...
#if defined(MIPS) && defined(OPT_SAFEPOINT)
  void * p = (void *)(0x10000);
