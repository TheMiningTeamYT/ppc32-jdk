...
void OptoRuntime::generate_exception_blob() {
  ...
  __ st(R4_ARG2/*exception pc*/, _abi(lr), R1_SP);

  // Store the exception in the Thread object.
  __ st(R3_ARG1/*exception oop*/, in_bytes(JavaThread::exception_oop_offset()), R16_thread);
  __ st(R4_ARG2/*exception pc*/,  in_bytes(JavaThread::exception_pc_offset()),  R16_thread);
...
# endif
  ...
  __ l(R3_ARG1, in_bytes(JavaThread::exception_oop_offset()), R16_thread);
  ...
  __ li(R0, 0);
#ifdef ASSERT
  __ st(R0, in_bytes(JavaThread::exception_handler_pc_offset()), R16_thread);
