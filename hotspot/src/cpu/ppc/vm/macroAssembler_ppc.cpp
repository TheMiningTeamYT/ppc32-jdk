...
// 2) compressed klass:
//    lis  rx = const.hi
//    clrli rx = rx & 0xFFFFffff // clearMS32b, optional
//    ori rx = rx | const.lo
...
bool MacroAssembler::is_load_const_from_method_toc_at(address a) {
  ...
  const int inst1 = *(int *)inst1_addr;
   ...
   // The relocation points to the l or the addis.
   return (is_ld(inst1)) ||
...
int MacroAssembler::get_offset_of_load_const_from_method_toc_at(address a) {
  ...
  if (is_ld(inst1)) {
    ...
    // Now, find the succeeding l which reads and writes to dst.
    ...
    while (true) {
      ...
      if (is_ld(inst2) && inv_ra_field(inst2) == dst && inv_rt_field(inst2) == dst) {
        // Stop, found the l which reads and writes dst.
        break;
      }
...
long MacroAssembler::get_const(address a) {
  ...
  return (long) x;
#else
  return (((unsigned long) (get_imm(a,0) & 0xffff)) << 16) |
          ((unsigned long) (get_imm(a,1) & 0xffff));
#endif
}

// Patch the constant of a `load_const' sequence. This is a low
...
#else
  set_imm(0 + p, (x >> 16) & 0xffff);
  set_imm(1 + p, x & 0xffff);
#endif
}
...
RegisterOrConstant MacroAssembler::delayed_value_impl(intptr_t* delayed_value_addr,
  ...
  l(tmp, simm16_offset, tmp); // must be aligned ((xa & 3) == 0)
...
//    _savegpr0_14:  st  r14,-144(r1)
//    _savegpr0_15:  st  r15,-136(r1)
//    _savegpr0_16:  st  r16,-128(r1)
void MacroAssembler::save_nonvolatile_gprs(Register dst, int offset) {
  st(R14, offset, dst);   offset += wordSize;
  st(R15, offset, dst);   offset += wordSize;
  st(R16, offset, dst);   offset += wordSize;
  st(R17, offset, dst);   offset += wordSize;
  st(R18, offset, dst);   offset += wordSize;
  st(R19, offset, dst);   offset += wordSize;
  st(R20, offset, dst);   offset += wordSize;
  st(R21, offset, dst);   offset += wordSize;
  st(R22, offset, dst);   offset += wordSize;
  st(R23, offset, dst);   offset += wordSize;
  st(R24, offset, dst);   offset += wordSize;
  st(R25, offset, dst);   offset += wordSize;
  st(R26, offset, dst);   offset += wordSize;
  st(R27, offset, dst);   offset += wordSize;
  st(R28, offset, dst);   offset += wordSize;
  st(R29, offset, dst);   offset += wordSize;
  st(R30, offset, dst);   offset += wordSize;
  st(R31, offset, dst);   offset += wordSize;

#ifndef USE_SPE
  stfd(F14, offset, dst);   offset += longSize;
  stfd(F15, offset, dst);   offset += longSize;
  stfd(F16, offset, dst);   offset += longSize;
  stfd(F17, offset, dst);   offset += longSize;
  stfd(F18, offset, dst);   offset += longSize;
  stfd(F19, offset, dst);   offset += longSize;
  stfd(F20, offset, dst);   offset += longSize;
  stfd(F21, offset, dst);   offset += longSize;
  stfd(F22, offset, dst);   offset += longSize;
  stfd(F23, offset, dst);   offset += longSize;
  stfd(F24, offset, dst);   offset += longSize;
  stfd(F25, offset, dst);   offset += longSize;
  stfd(F26, offset, dst);   offset += longSize;
  stfd(F27, offset, dst);   offset += longSize;
  stfd(F28, offset, dst);   offset += longSize;
  stfd(F29, offset, dst);   offset += longSize;
  stfd(F30, offset, dst);   offset += longSize;
  stfd(F31, offset, dst);
#endif
}

// Uses ordering which corresponds to ABI:
//    _restgpr0_14:  l   r14,-144(r1)
//    _restgpr0_15:  l   r15,-136(r1)
//    _restgpr0_16:  l   r16,-128(r1)
void MacroAssembler::restore_nonvolatile_gprs(Register src, int offset) {
  l(R14, offset, src);   offset += wordSize;
  l(R15, offset, src);   offset += wordSize;
  l(R16, offset, src);   offset += wordSize;
  l(R17, offset, src);   offset += wordSize;
  l(R18, offset, src);   offset += wordSize;
  l(R19, offset, src);   offset += wordSize;
  l(R20, offset, src);   offset += wordSize;
  l(R21, offset, src);   offset += wordSize;
  l(R22, offset, src);   offset += wordSize;
  l(R23, offset, src);   offset += wordSize;
  l(R24, offset, src);   offset += wordSize;
  l(R25, offset, src);   offset += wordSize;
  l(R26, offset, src);   offset += wordSize;
  l(R27, offset, src);   offset += wordSize;
  l(R28, offset, src);   offset += wordSize;
  l(R29, offset, src);   offset += wordSize;
  l(R30, offset, src);   offset += wordSize;
  l(R31, offset, src);   offset += wordSize;

#ifndef USE_SPE
  // FP registers
  lfd(F14, offset, src);   offset += longSize;
  lfd(F15, offset, src);   offset += longSize;
  lfd(F16, offset, src);   offset += longSize;
  lfd(F17, offset, src);   offset += longSize;
  lfd(F18, offset, src);   offset += longSize;
  lfd(F19, offset, src);   offset += longSize;
  lfd(F20, offset, src);   offset += longSize;
  lfd(F21, offset, src);   offset += longSize;
  lfd(F22, offset, src);   offset += longSize;
  lfd(F23, offset, src);   offset += longSize;
  lfd(F24, offset, src);   offset += longSize;
  lfd(F25, offset, src);   offset += longSize;
  lfd(F26, offset, src);   offset += longSize;
  lfd(F27, offset, src);   offset += longSize;
  lfd(F28, offset, src);   offset += longSize;
  lfd(F29, offset, src);   offset += longSize;
  lfd(F30, offset, src);   offset += longSize;
  lfd(F31, offset, src);
#endif
}
...
#ifdef PPC64
  st(R2,  offset, dst);   offset += wordSize;
#endif
  st(R3,  offset, dst);   offset += wordSize;
  st(R4,  offset, dst);   offset += wordSize;
  st(R5,  offset, dst);   offset += wordSize;
  st(R6,  offset, dst);   offset += wordSize;
  st(R7,  offset, dst);   offset += wordSize;
  st(R8,  offset, dst);   offset += wordSize;
  st(R9,  offset, dst);   offset += wordSize;
  st(R10, offset, dst);   offset += wordSize;
  st(R11, offset, dst);   offset += wordSize;
  st(R12, offset, dst);   offset += wordSize;

#ifndef USE_SPE
  stfd(F0, offset, dst);   offset += longSize;
  stfd(F1, offset, dst);   offset += longSize;
  stfd(F2, offset, dst);   offset += longSize;
  stfd(F3, offset, dst);   offset += longSize;
  stfd(F4, offset, dst);   offset += longSize;
  stfd(F5, offset, dst);   offset += longSize;
  stfd(F6, offset, dst);   offset += longSize;
  stfd(F7, offset, dst);   offset += longSize;
  stfd(F8, offset, dst);   offset += longSize;
  stfd(F9, offset, dst);   offset += longSize;
  stfd(F10, offset, dst);  offset += longSize;
  stfd(F11, offset, dst);  offset += longSize;
  stfd(F12, offset, dst);  offset += longSize;
  stfd(F13, offset, dst);
#endif
}
...
#ifdef PPC64
  l(R2,  offset, src);   offset += wordSize;
#endif
  l(R3,  offset, src);   offset += wordSize;
  l(R4,  offset, src);   offset += wordSize;
  l(R5,  offset, src);   offset += wordSize;
  l(R6,  offset, src);   offset += wordSize;
  l(R7,  offset, src);   offset += wordSize;
  l(R8,  offset, src);   offset += wordSize;
  l(R9,  offset, src);   offset += wordSize;
  l(R10, offset, src);   offset += wordSize;
  l(R11, offset, src);   offset += wordSize;
  l(R12, offset, src);   offset += wordSize;

#ifndef USE_SPE
  lfd(F0, offset, src);   offset += longSize;
  lfd(F1, offset, src);   offset += longSize;
  lfd(F2, offset, src);   offset += longSize;
  lfd(F3, offset, src);   offset += longSize;
  lfd(F4, offset, src);   offset += longSize;
  lfd(F5, offset, src);   offset += longSize;
  lfd(F6, offset, src);   offset += longSize;
  lfd(F7, offset, src);   offset += longSize;
  lfd(F8, offset, src);   offset += longSize;
  lfd(F9, offset, src);   offset += longSize;
  lfd(F10, offset, src);  offset += longSize;
  lfd(F11, offset, src);  offset += longSize;
  lfd(F12, offset, src);  offset += longSize;
  lfd(F13, offset, src);
#endif
}
...
void MacroAssembler::save_LR_CR(Register tmp) {
  mfcr(tmp);
  st(tmp, _abi(cr), R1_SP);
  mflr(tmp);
  st(tmp, _abi(lr), R1_SP);
...
void MacroAssembler::resize_frame(Register offset, Register tmp) {
  ...
  // tmp <- *(SP)
  l(tmp, _abi(callers_sp), R1_SP);
  // addr <- SP + offset;
  // *(addr) <- tmp;
  // SP <- addr
  stux(tmp, R1_SP, offset);
}
...
address MacroAssembler::branch_to(Register function_descriptor, bool and_link, bool save_toc_before_call,
  ...
  l(R0, in_bytes(FunctionDescriptor::entry_offset()), function_descriptor);
  mtctr(R0);

  if (load_toc_of_callee) {
    l(R2_TOC, in_bytes(FunctionDescriptor::toc_offset()), function_descriptor);
  }
  if (load_env_of_callee) {
    l(R11, in_bytes(FunctionDescriptor::env_offset()), function_descriptor);
...
bool MacroAssembler::is_load_from_polling_page(int instruction, void* ucontext,
  ...
  if (!is_ld(instruction))
    return false; // It's not a l. Fail.

  int rt = inv_rt_field(instruction);
  int ra = inv_ra_field(instruction);
  int ds = inv_ds_field(instruction);
  if (!(ds == 0 && ra != 0 && rt == 0)) {
    return false; // It's not a l(r0, X, ra). Fail.
  }
...
void MacroAssembler::bang_stack_with_offset(int offset) {
  ...
  intptr_t stdoffset = -offset;

  if (is_simm(stdoffset, 16)) {
    // Signed 16 bit offset, a simple st is ok.
    if (UseLoadInstructionsForStackBangingPPC64) {
      l(R0, (int)(signed short)stdoffset, R1_SP);
    } else {
      st(R0,(int)(signed short)stdoffset, R1_SP);
    }
...
//    st    R0,    x(Ry),       (see bang_stack_with_offset())
//    stu   R1_SP, x(R1_SP),    (see push_frame(), resize_frame())
// or stux  R1_SP, Rx, R1_SP    (see push_frame(), resize_frame())
...
void MacroAssembler::cmpxchgd(ConditionRegister flag,
  ...
  // Add simple guard in order to reduce risk of starving under high contention (recommended by IBM).
  if (contention_hint) { // Don't try to reserve if cmp fails.
    l(dest_current_value, 0, addr_base);
    cmp(flag, dest_current_value, compare_value);
    bne(flag, failed);
  }
  ...
  larx(dest_current_value, addr_base, cmpxchgx_hint);
  cmp(flag, dest_current_value, compare_value);
  if (UseStaticBranchPredictionInCompareAndSwapPPC64) {
    bne_predict_not_taken(flag, failed);
  } else {
  ...
  stcx_(exchange_value, addr_base);
  if (UseStaticBranchPredictionInCompareAndSwapPPC64) {
    bne_predict_not_taken(CCR0, retry); // stXcx_ sets CCR0
  } else {
    bne(                  CCR0, retry); // stXcx_ sets CCR0
...
void MacroAssembler::lookup_interface_method(Register recv_klass,
  ...
  if (HeapWordsPerLong > 1) {
    ...
    round_to(scan_temp, HeapWordsPerLong);
  }

  sli(scan_temp, scan_temp, log_vte_size);
  ...
  if (return_method) {
    if (itable_index.is_register()) {
      ...
      sli(method_result, itable_offset, logMEsize);
  ...
  for (int peel = 1; peel >= 0; peel--) {
    // %%%% Could load both offset and interface in one lx, if they were
    // in the opposite order. This would save a load.
    l(temp2, itableOffsetEntry::interface_offset_in_bytes(), scan_temp);

    // Check that this entry is non-null. A null entry means that
    // the receiver class doesn't implement the interface, and wasn't the
    // same as when the caller was compiled.
    cmp(CCR0, temp2, intf_klass);
  ...
  if (return_method) {
    ...
    lx(method_result, scan_temp, method_result);
  }
}
...
void MacroAssembler::lookup_virtual_method(Register recv_klass,
  ...
  if (vtable_index.is_register()) {
    sli(vtable_index.as_register(), vtable_index.as_register(), LogBytesPerWord);
    add(recv_klass, vtable_index.as_register(), recv_klass);
  } else {
    addi(recv_klass, recv_klass, vtable_index.as_constant() << LogBytesPerWord);
  }
  l(R19_method, base + vtableEntry::method_offset_in_bytes(), recv_klass);
}
...
void MacroAssembler::check_klass_subtype_fast_path(Register sub_klass,
  ...
  cmp(CCR0, sub_klass, super_klass);
  beq(CCR0, *L_success);
  ...
  l(cached_super, super_check_offset, sub_klass);
  cmp(CCR0, cached_super, super_klass);
...
void MacroAssembler::check_klass_subtype_slow_path(Register sub_klass,
  ...
  l(array_ptr, source_offset, sub_klass);
  ...
  l(temp, base_offset, array_ptr);
  cmp(CCR0, temp, super_klass);
  beq(CCR0, hit);
  ...
  bind(hit);
  st(super_klass, target_offset, sub_klass); // save result to cache
...
RegisterOrConstant MacroAssembler::argument_offset(RegisterOrConstant arg_slot,
  ...
  if (arg_slot.is_constant()) {
    ...
    sli(temp_reg, arg_slot.as_register(), exact_log2(stackElementSize));
    if (offset != 0)
...
void MacroAssembler::biased_locking_enter(ConditionRegister cr_reg, Register obj_reg,
  ...
  if (PrintBiasedLockingStatistics) {
    ...
    // restore mark_reg
    l(mark_reg, oopDesc::mark_offset_in_bytes(), obj_reg);
    bind(lbl);
  }
  beq(cr_reg, done);
  ...
  rlicl_(temp2_reg, temp_reg, shift_amount, 64 - markOopDesc::epoch_bits);
...
void MacroAssembler::tlab_allocate(
  ...
  l(obj, in_bytes(JavaThread::tlab_top_offset()), R16_thread);
  l(R0, in_bytes(JavaThread::tlab_end_offset()), R16_thread);
  if (var_size_in_bytes == noreg) {
    addi(new_top, obj, con_size_in_bytes);
  } else {
    add(new_top, obj, var_size_in_bytes);
  }
  cmpl(CCR0, new_top, R0);
  ...
  st(new_top, in_bytes(JavaThread::tlab_top_offset()), R16_thread);
  //verify_tlab(); not implemented
}
...
void MacroAssembler::compiler_fast_lock_object(ConditionRegister flag, Register oop, Register box,
  ...
  l(displaced_header, oopDesc::mark_offset_in_bytes(), oop);


  // Always do locking in runtime.
  if (EmitSync & 0x01) {
    cmpi(flag, oop, 0); // Oop can't be 0 here => always false.
    return;
  }
  ...
  st(displaced_header, BasicLock::displaced_header_offset_in_bytes(), box);
  ...
  mcrf(flag,CCR0);
  st(R0/*==0, perhaps*/, BasicLock::displaced_header_offset_in_bytes(), box);
  ...
  if ((EmitSync & 0x02) == 0) {
    ...
    st(box, BasicLock::displaced_header_offset_in_bytes(), box);
...
void MacroAssembler::compiler_fast_unlock_object(ConditionRegister flag, Register oop, Register box,
  ...
  l(displaced_header, BasicLock::displaced_header_offset_in_bytes(), box);

  // If the displaced header is 0, we have a recursive unlock.
  cmpi(flag, displaced_header, 0);
  beq(flag, cont);

  // Handle existing monitor.
  if ((EmitSync & 0x02) == 0) {
    // The object has an existing monitor iff (mark & monitor_value) != 0.
    l(current_header, oopDesc::mark_offset_in_bytes(), oop);
  ...
  if ((EmitSync & 0x02) == 0) {
    ...
    l(temp,             ObjectMonitor::owner_offset_in_bytes(), current_header);
    l(displaced_header, ObjectMonitor::recursions_offset_in_bytes(), current_header);
    ...
    l(temp,             ObjectMonitor::EntryList_offset_in_bytes(), current_header);
    l(displaced_header, ObjectMonitor::cxq_offset_in_bytes(), current_header);
    orr(temp, temp, displaced_header); // Will be 0 if both are 0.
    cmpi(flag, temp, 0);
    bne(flag, cont);
    release();
    st(temp, ObjectMonitor::owner_offset_in_bytes(), current_header);
  }
...
void MacroAssembler::serialize_memory(Register thread, Register tmp1, Register tmp2) {
  sri(tmp2, thread, os::get_serialize_page_shift_count());
...
void MacroAssembler::card_table_write(jbyte* byte_map_base, Register Rtmp, Register Robj) {
  ...
  sri(Robj, Robj, CardTableModRefBS::card_shift);
  li(R0, 0); // dirty
...
void MacroAssembler::resolve_jobject(Register value, Register tmp1, Register tmp2, bool needs_frame) {
  ...
  clrri(tmp1, value, JNIHandles::weak_tag_size);
#if INCLUDE_ALL_GCS
  if (UseG1GC) { andi_(tmp2, value, JNIHandles::weak_tag_mask); }
#endif
  l(value, 0, tmp1);      // Resolve (untagged) jobject.
...
void MacroAssembler::g1_write_barrier_pre(Register Robj, RegisterOrConstant offset, Register Rpre_val,
  ...
  l(Rindex, in_bytes(JavaThread::satb_mark_queue_offset() + PtrQueue::byte_offset_of_index()), R16_thread);
  cmpi(CCR0, Rindex, 0);
  beq(CCR0, runtime); // If index == 0, goto runtime.
  l(Rbuffer, in_bytes(JavaThread::satb_mark_queue_offset() + PtrQueue::byte_offset_of_buf()), R16_thread);

  addi(Rindex, Rindex, -wordSize); // Decrement index.
  st(Rindex, in_bytes(JavaThread::satb_mark_queue_offset() + PtrQueue::byte_offset_of_index()), R16_thread);

  // Record the previous value.
  stx(Rpre_val, Rbuffer, Rindex);
  b(filtered);
...
void MacroAssembler::g1_write_barrier_post(Register Rstore_addr, Register Rnew_val, Register Rtmp1, Register Rtmp2, Register Rtmp3, Label *filtered_ext) {
  ...
  if (G1RSBarrierRegionFilter) {
    ...
    sri_(Rtmp1, Rtmp1, HeapRegion::LogOfHRGrainBytes);
    beq(CCR0, filtered);
  }
  ...
  sri(Rcard_addr, Rstore_addr, CardTableModRefBS::card_shift);
  ...
  const Register Rqueue_index = Rtmp2,
                 Rqueue_buf   = Rtmp3;
  l(Rqueue_index, in_bytes(JavaThread::dirty_card_queue_offset() + PtrQueue::byte_offset_of_index()), R16_thread);
  cmpi(CCR0, Rqueue_index, 0);
  beq(CCR0, runtime); // index == 0 then jump to runtime
  l(Rqueue_buf, in_bytes(JavaThread::dirty_card_queue_offset() + PtrQueue::byte_offset_of_buf()), R16_thread);

  addi(Rqueue_index, Rqueue_index, -wordSize); // decrement index
  st(Rqueue_index, in_bytes(JavaThread::dirty_card_queue_offset() + PtrQueue::byte_offset_of_index()), R16_thread);

  stx(Rcard_addr, Rqueue_buf, Rqueue_index); // store card
  b(filtered);
...
void MacroAssembler::set_last_Java_frame(Register last_Java_sp, Register last_Java_pc) {
  ...
  if (last_Java_pc != noreg)
    st(last_Java_pc, in_bytes(JavaThread::last_Java_pc_offset()), R16_thread);

  // Set last_Java_sp last.
  st(last_Java_sp, in_bytes(JavaThread::last_Java_sp_offset()), R16_thread);
}
...
void MacroAssembler::reset_last_Java_frame(void) {
  ...
  // _last_Java_sp = 0
  st(R0, in_bytes(JavaThread::last_Java_sp_offset()), R16_thread);

  // _last_Java_pc = 0
  st(R0, in_bytes(JavaThread::last_Java_pc_offset()), R16_thread);
  BLOCK_COMMENT("} reset_last_Java_frame");
}
...
#ifdef CC_INTERP
  l(tmp1/*pc*/, _top_ijava_frame_abi(frame_manager_lr), sp);
#else
  address entry = pc();
...
void MacroAssembler::get_vm_result(Register oop_result) {
  ...
  l(oop_result, in_bytes(JavaThread::vm_result_offset()), R16_thread);
  li(R0, 0);
...
void MacroAssembler::get_vm_result_2(Register metadata_result) {
  ...
  l(metadata_result, in_bytes(JavaThread::vm_result_2_offset()), R16_thread);
  li(R0, 0);
...
void MacroAssembler::encode_klass_not_null(Register dst, Register src) {
  ...
  if (Universe::narrow_klass_shift() != 0) {
    sri(dst, current, Universe::narrow_klass_shift());
    current = dst;
  }
#endif
...
void MacroAssembler::store_klass(Register dst_oop, Register klass, Register ck) {
  ...
  if (UseCompressedClassPointers) {
    ...
    st(klass, oopDesc::klass_offset_in_bytes(), dst_oop);
  }
#else
  st(klass, oopDesc::klass_offset_in_bytes(), dst_oop);
#endif
...
void MacroAssembler::decode_klass_not_null(Register dst, Register src) {
  ...
  if (Universe::narrow_klass_shift() != 0 ||
    ...
    shifted_src = dst;
    sli(shifted_src, src, Universe::narrow_klass_shift());
  }
  if (Universe::narrow_klass_base() != 0) {
...
void MacroAssembler::load_klass(Register dst, Register src) {
  if (UseCompressedClassPointers) {
    ...
    l(dst, oopDesc::klass_offset_in_bytes(), src);
  }
}
...
#ifdef PPC64
  assert(d == R30, "R30 is the only heap base");
...
void MacroAssembler::clear_memory_word(Register base_ptr, Register cnt_words, Register tmp) {
  // Procedure for large arrays (uses data cache block zero instruction).
    Label startloop, fast, fastloop, small_rest, restloop, done;
    const int cl_size         = VM_Version::get_cache_line_size(),
              cl_words       = cl_size >> LogBytesPerWord,
              cl_w_addr_bits = exact_log2(cl_words),
              dcbz_min        = 1;                     // Min count of dcbz executions, needs to be >0.

//2:
    cmpi(CCR1, cnt_words, ((dcbz_min+1)<<cl_w_addr_bits)-1); // Big enough? (ensure >=dcbz_min lines included).
    blt(CCR1, small_rest);                                      // Too small.
    rlicl_(tmp, base_ptr, 64-LogBytesPerWord, 64-cl_w_addr_bits);           // Extract dword offset within first cache line.
    beq(CCR0, fast);                                            // Already 128byte aligned.

    subfic(tmp, tmp, cl_words);
    mtctr(tmp);                        // Set ctr to hit 128byte boundary (0<ctr<cl_dwords).
    subf(cnt_words, tmp, cnt_words); // rest.
    li(tmp, 0);
//10:
  bind(startloop);                     // Clear at the beginning to reach 128byte boundary.
    st(tmp, 0, base_ptr);             // Clear 8byte aligned block.
    addi(base_ptr, base_ptr, BytesPerWord);
    bdnz(startloop);
//13:
  bind(fast);                                  // Clear 128byte blocks.
    sri(tmp, cnt_words, cl_w_addr_bits);    // Loop count for 128byte loop (>0).
    andi(cnt_words, cnt_words, cl_words-1); // Rest in dwords.
...
//20:
  bind(small_rest);
    cmpi(CCR0, cnt_words, 0);        // size 0?
    beq(CCR0, done);                   // rest == 0
    li(tmp, 0);
    mtctr(cnt_words);                 // Load counter.
//24:
...
//24:
  ...
  bind(L_Found3);                  // Return index ...
   ...
   sri(result, addr, 1);      // in characters.
  bind(L_End);
}
...
//24:
  bind(L_FinalCheck);
   rlicl_(addr_diff, addr_diff, 64-1, 63); // Remaining characters not covered by InnerLoop: (addr_diff>>1)&1.
   beq(CCR0, L_NotFound);
...
//16:
   ...
   bind(Lfast_loop);
    l(chr1_reg, 0, str1_reg);
    lx(chr2_reg, str1_reg, addr_diff);
    cmp(CCR0, chr2_reg, chr1_reg);
...
//8:
    bind(Lloop);
      lx(tmp1_reg, str1_reg, index_reg);
      lx(tmp2_reg, str2_reg, index_reg);
      cmp(CCR0, tmp1_reg, tmp2_reg);
...
void MacroAssembler::char_arrays_equalsImm(Register str1_reg, Register str2_reg, int cntval, Register result_reg,
  ...
  if (cntval < 16) { // short case
    ...
    for (int next_index; (next_index = index + 8) <= num_bytes; index = next_index) {
      l(tmp1_reg, index, str1_reg);
      l(tmp2_reg, index, str2_reg);
      cmp(CCR0, tmp1_reg, tmp2_reg);
    ...
    //8:
    bind(Lloop);
    lx(R0, str1_reg, index_reg);
    lx(tmp2_reg, str2_reg, index_reg);
    cmp(CCR0, R0, tmp2_reg);
    bne(CCR0, Ldone_false);  // Unequal char pair found -> done.
    addi(index_reg, index_reg, 4*sizeof(jchar));
    bdnz(Lloop);
    //14:
...
#ifdef ASSERT
static void vm_asm_printf(const char *fmt, int a1, int a2, int a3) {
  fprintf(stderr, fmt, a1, a2, a3);
}
#endif // ASSERT

void MacroAssembler::asm_printf(const char *fmt, Register a1, Register a2, Register a3) {
#ifdef ASSERT
  const Register tmp = R11; // Will be preserved.
  const int nbytes_save = ::round_to(volatile_regs_size, StackAlignmentInBytes); // Volatile gprs with R0.
  const int first_volatile_enc = PPC64_ONLY(R2->encoding()) NOT_PPC64(R3->encoding());
  save_volatile_gprs(R1_SP, -nbytes_save); // except R0
  // save R0
  mr(tmp, R0);
  st(tmp, -wordSize, R1_SP);
  ...
  load_const_optimized(R3, (address)fmt);

#define PASS_ARG(Rdst, Rsrc) do { \
    if (Rsrc != noreg) { \
      NOT_PPC64(assert(first_volatile_enc != R2->encoding(), "R2 is not saved in PPC32")); \
      if (Rsrc->encoding() > R12->encoding()) { \
        mr(Rdst, Rsrc); \
      } else if (Rsrc->encoding() >= first_volatile_enc) { \
        l(Rdst, frame::abi_reg_args_size + (Rsrc->encoding() - first_volatile_enc) * wordSize, R1_SP); \
      } else if (Rsrc == R0) { \
        l(Rdst, frame::abi_reg_args_size + nbytes_save - wordSize, R1_SP); \
      } else if (Rsrc == R1_SP) { \
        addi(Rdst, R1_SP, frame::abi_reg_args_size + nbytes_save); \
      } else { \
        ShouldNotReachHere(); \
      } \
    } \
...
#if !defined(PPC64) || defined(ABI_ELFv2)
  call_c(CAST_FROM_FN_PTR(address, vm_asm_printf), relocInfo::none);
#else
  call_c(CAST_FROM_FN_PTR(FunctionDescriptor*, vm_asm_printf), relocInfo::none);
#endif
...
#endif
  // restore R0
  l(tmp, -wordSize, R1_SP);
  mr(R0, tmp);

  restore_volatile_gprs(R1_SP, -nbytes_save);
...
void MacroAssembler::os_breakpoint() {
  const Register tmp = R11; // Will be preserved.
  // ppc32: alignment required to push R0 with volatile regs
  const int nbytes_save = MacroAssembler::volatile_regs_size + wordSize NOT_PPC64(*2); // Volatile gprs with R0.
  save_volatile_gprs(R1_SP, -nbytes_save); // except R0
  // save R0
  mr(tmp, R0);
  st(tmp, -wordSize, R1_SP);
  ...
  push_frame_reg_args(nbytes_save, tmp);
#if !defined(PPC64) || defined(ABI_ELFv2)
  call_c(CAST_FROM_FN_PTR(address, os::breakpoint), relocInfo::none);
#else
  call_c(CAST_FROM_FN_PTR(FunctionDescriptor*, os::breakpoint), relocInfo::none);
#endif
  pop_frame();
#ifdef PPC64
  restore_LR_CR(tmp);
...
void MacroAssembler::zap_from_to(Register low, int before, Register high, int after, Register val, Register addr) {
  ...
  load_const_optimized(val, PPC64_ONLY(0x0101010101010101) NOT_PPC64(0x01010101));
...
void MacroAssembler::cmp(ConditionRegister d, RegisterOrConstant roc, Register s1) {
  if (roc.is_constant()) {
    intptr_t c = roc.as_constant();
    assert(is_simm(c, 16), "too big");
    cmpi(d, s1, (int)c);
  }
  else cmp(d, roc.as_register(), s1);
}
