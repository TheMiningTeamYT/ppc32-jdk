...
void frame::patch_pc(Thread* thread, address pc) {
  if (TracePcPatching) {
    tty->print_cr("patch_pc at address " PTR_FORMAT " [" PTR_FORMAT " -> " PTR_FORMAT "]",
                  p2i(&own_abi()->lr), own_abi()->lr, p2i(pc));
  }
...
BasicType frame::interpreter_frame_result(oop* oop_result, jvalue* value_result) {
  ...
  if (method->is_native()) {
    ...
    switch (method->result_type()) {
      ...
      // We use st/stfd to store the values.
      ...
      case T_LONG    : value_result->j = (jlong)    *(PPC64_ONLY(long) NOT_PPC64(long long)*)lresult; break;
      case T_FLOAT   : value_result->f = (jfloat)   NOT_USE_SPE(*(double*)fresult) USE_SPE_ONLY(*(float*) lresult); break;
      case T_DOUBLE  : value_result->d = (jdouble)  NOT_USE_SPE(*(double*)fresult) USE_SPE_ONLY(*(double*)lresult); break;
