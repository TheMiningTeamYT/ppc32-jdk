...
void CounterOverflowStub::emit_code(LIR_Assembler* ce) {
  ...
  __ st(R0, -BytesPerWord, R1_SP);
#else
  __ st(_method->as_register(), -BytesPerWord, R1_SP);
#endif
...
void MonitorEnterStub::emit_code(LIR_Assembler* ce) {
  ...
  __ mr_if_needed(R5_ARG3, _lock_reg->as_register());
  __ mtctr(R0);
  __ bctrl();
  ce->add_call_info_here(_info);
  ce->verify_oop_map(_info);
...
int PatchingStub::_patch_info_offset = -(PPC64_ONLY(5) NOT_PPC64(4) * BytesPerInstWord);
...
void PatchingStub::emit_code(LIR_Assembler* ce) {
  ...
  if (_id == load_mirror_id) {
    ...
    __ mr(R0, _obj); // spill
    __ l(_obj, java_lang_Class::klass_offset_in_bytes(), _obj);
    __ l(_obj, in_bytes(InstanceKlass::init_thread_offset()), _obj);
    __ cmp(CCR0, _obj, R16_thread);
...
#ifdef PPC64
  // To initial ppc64 code refer to
  // cd20533d507c: [c1] taking c1 related files from 9751:4a24de859a87
  Unimplemented();
#else
    __ bind(_entry);
    __ call_c(Runtime1::entry_for(Runtime1::deoptimize_id), relocInfo::runtime_call_type);
    ce->add_call_info_here(_info);
...
void G1PreBarrierStub::emit_code(LIR_Assembler* ce) {
  ...
  __ load_const_optimized(R0, stub);
#endif
  __ st(pre_val_reg, -BytesPerWord, R1_SP); // Pass pre_val on stack.
  __ mtctr(R0);
  __ bctrl();
  __ b(_continuation);
}
