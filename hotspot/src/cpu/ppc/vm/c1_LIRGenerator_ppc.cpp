...
LIR_Opr LIRGenerator::result_register_for(ValueType* type, bool callee) {
  ...
  case longTag:    opr = FrameMap::PPC64_ONLY(R3_long_opr) NOT_PPC64(R3_R4_long_opr); break;
  case floatTag:   opr = NOT_USE_SPE(FrameMap::F1_opr) USE_SPE_ONLY(FrameMap::R3_float_opr); break;
  case doubleTag:  opr = NOT_USE_SPE(FrameMap::F1_double_opr) USE_SPE_ONLY(FrameMap::R3_R4_double_opr); break;
  case addressTag:
...
#ifdef USE_SPE
  if (type->tag() == doubleTag) {
    assert(FrameMap::R3_double_opr->type_field() == LIR_OprDesc::double_type,
        "type mismatch");
    return FrameMap::R3_double_opr;
  }
#endif
  return result_register_for(type, callee);
}
#endif
...
LIR_Address* LIRGenerator::generate_address(LIR_Opr base, LIR_Opr index,
  ...
  if (index->is_register()) {
    ...
    if (disp != 0) {
      ...
      USE_SPE_ONLY( || (type == T_DOUBLE && !Assembler::is_ev_uimm8_5(disp)))) {
...
LIR_Address* LIRGenerator::emit_array_address(LIR_Opr array_opr, LIR_Opr index_opr,
  ...
  if (index_opr->is_constant()) {
    ...
    if (Assembler::is_simm16(array_offset + offset)
        USE_SPE_ONLY( && (type != T_DOUBLE || Assembler::is_ev_uimm8_5(array_offset + offset)))) {
      base_opr = array_opr;
...
bool LIRGenerator::strength_reduce_multiply(LIR_Opr left, jint c, LIR_Opr result, LIR_Opr tmp) {
  ...
  if (tmp->is_valid() && c > 0 && c < max_jint) {
    if (is_power_of_2(c + 1)) {
      __ shift_left(left, log2_intptr((intptr_t)(c + 1)), result);
...
void LIRGenerator::do_ArithmeticOp_Long(ArithmeticOp* x) {
  ...
  if (is_div_rem) {
    ...
    jlong const_divisor = 0;
    if (right.value()->type()->as_LongConstant()) {
      const_divisor = right.value()->type()->as_LongConstant()->value();
    } else if (right.value()->type()->as_IntConstant()) {
      const_divisor = right.value()->type()->as_IntConstant()->value();
    }
...
#ifndef PPC64
    if (divisor->is_register() || !is_power_of_2(const_divisor<0?-const_divisor:const_divisor)) {
      address entry = x->op() == Bytecodes::_ldiv ?
  ...
  if (is_div_rem) {
    ...
    LIR_Opr tmp = PPC64_ONLY(FrameMap::R0_opr) NOT_PPC64(new_register(T_LONG));
    ...
    // arithmetic_op_long is not suitable here since doesn't accept tmp Long Opr
    arithmetic_op(x->op(), x->operand(), left.result(), right.result(), false, FrameMap::R0_opr);
  }
}
...
void LIRGenerator::do_ArithmeticOp_Int(ArithmeticOp* x) {
  ...
  if (is_div_rem) {
    ...
    if (x->op() == Bytecodes::_irem) {
      __ irem(left.result(), right.result(), x->operand(), new_register(T_INT), info);
...
void LIRGenerator::do_ShiftOp(ShiftOp* x) {
  ...
  if (count.result()->is_register()) {
    mcount = PPC64_ONLY(FrameMap::R0_opr) NOT_PPC64(new_register(T_INT));
  } else {
...
#else
  const bool ll = Assembler::is_uimm(int_or_long_const, 16);
  const bool lh = (int_or_long_const & 0xFFFF) == 0;
  const bool hl = Assembler::is_uimm(int_or_long_const >> 32, 16);
  const bool hh = ((int_or_long_const >> 32) & 0xFFFF) == 0;
  if ((ll || lh) && (hl || hh)) {
    return true;
  }
#endif

  // see Assembler::andi; don't use this on PPC32, see c1_LIRAssembler::shift_op
#if defined(PPC64)
...
#else
  if ((bc == Bytecodes::_ixor || bc == Bytecodes::_lxor) &&
    ((((int_or_long_const & 0xFFFFFFFF) == 0xFFFFFFFF) && (hl || hh)) ||
     (((int_or_long_const >> 32)        == 0xFFFFFFFF) && (ll || lh)) )) {
    return true;
  }
#endif
...
void LIRGenerator::do_Convert(Convert* x) {
  switch (x->op()) {
    ...
    case Bytecodes::_l2d: {
      address entry = CAST_FROM_FN_PTR(address, SharedRuntime::l2d);
      LIR_Opr result = call_runtime(x->value(), entry, x->type(), NULL);
      set_result(x, result);
      break;
    }
    case Bytecodes::_f2l: {
      address entry = CAST_FROM_FN_PTR(address, SharedRuntime::f2l);
      LIR_Opr result = call_runtime(x->value(), entry, x->type(), NULL);
      set_result(x, result);
      break;
    }
    case Bytecodes::_d2l: {
     address entry = CAST_FROM_FN_PTR(address, SharedRuntime::d2l);
      LIR_Opr result = call_runtime(x->value(), entry, x->type(), NULL);
      set_result(x, result);
      break;
    }
#ifndef USE_SPE
    case Bytecodes::_i2f: {
      address entry = CAST_FROM_FN_PTR(address, SharedRuntime::i2f);
      LIR_Opr result = call_runtime(x->value(), entry, x->type(), NULL);
      set_result(x, result);
      break;
    }
    case Bytecodes::_i2d: {
      address entry = CAST_FROM_FN_PTR(address, SharedRuntime::i2d);
      LIR_Opr result = call_runtime(x->value(), entry, x->type(), NULL);
      set_result(x, result);
      break;
    }
    case Bytecodes::_f2i: {
      address entry = CAST_FROM_FN_PTR(address, SharedRuntime::f2i);
      LIR_Opr result = call_runtime(x->value(), entry, x->type(), NULL);
      set_result(x, result);
      break;
    }
    case Bytecodes::_d2i: {
      address entry = CAST_FROM_FN_PTR(address, SharedRuntime::d2i);
      LIR_Opr result = call_runtime(x->value(), entry, x->type(), NULL);
      set_result(x, result);
     break;
    }
...
void LIRGenerator::do_NewInstance(NewInstance* x) {
  ...
  const LIR_Opr reg = java_result_register_for(x->type());
#ifndef PRODUCT
...
void LIRGenerator::do_NewTypeArray(NewTypeArray* x) {
  ...
  LIR_Opr reg = java_result_register_for(x->type());
...
#ifndef USE_SPE
    LIR_Address *double_addr = new LIR_Address(address->base(),
        address->index(), address->scale(),
        address->disp(), T_DOUBLE);
    // store not volatile long to spill slot, load it as double and make
    // volatile store of double
    LIR_Opr temp_double = new_register(T_DOUBLE);
    LIR_Opr spill = new_register(T_LONG);
    set_vreg_flag(spill, must_start_in_memory);
    __ move(value, spill);
    __ move(spill, temp_double);
    __ store(temp_double, double_addr, info);
#else
    if (address->index() == LIR_OprFact::illegalOpr &&
            !Assembler::is_uimm(address->disp(), 8)) {
      // Adjust address if disp is bigger then 8 bits
      LIR_Opr adjusted_base = new_pointer_register();
      __ add(address->base(), load_immediate(address->disp(), T_INT), adjusted_base);
      LIR_Address *adjusted_addr = new LIR_Address( adjusted_base,
          address->index(), address->scale(), 0, T_LONG);
      __ volatile_store_mem_reg(value, adjusted_addr, info);
    } else
...
void LIRGenerator::volatile_field_load(LIR_Address* address, LIR_Opr result,
  ...
  if (address->type() == T_LONG) {
#ifndef USE_SPE
    // load volatile long as double, save it in spill slot and load it again
    // as not volatile long
    LIR_Address *double_addr = new LIR_Address(address->base(),
        address->index(), address->scale(),
        address->disp(), T_DOUBLE);
    LIR_Opr temp_double = new_register(T_DOUBLE);
    __ load(double_addr, temp_double, info);
    __ move(temp_double, result);
    ...
    if (address->index() == LIR_OprFact::illegalOpr &&
      ...
      LIR_Address *adjusted_addr = new LIR_Address( adjusted_base,
          address->index(), address->scale(), 0, T_LONG);
      __ volatile_load_mem_reg(adjusted_addr, result, info);
    } else
...
void LIRGenerator::put_Object_unsafe(LIR_Opr src, LIR_Opr offset, LIR_Opr data,
  ...
  if (is_volatile && type == T_LONG) {
    volatile_field_store(data, addr, NULL);
  } else
#endif
  {
    __ store(data, addr);
...
#ifndef _LP64
  if (is_volatile && type == T_LONG) {
    volatile_field_load(addr, dst, NULL);
  } else
#endif
  {
    __ load(addr, dst);
