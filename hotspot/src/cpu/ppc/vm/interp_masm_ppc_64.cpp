...
void InterpreterMacroAssembler::dispatch_prolog(TosState state, int bcp_incr) {
  ...
  sli(bytecode, bytecode, LogBytesPerWord);
  lx(R24_dispatch_addr, R24_dispatch_addr, bytecode);
}
...
void InterpreterMacroAssembler::check_and_handle_earlyret(Register scratch_reg) {
  ...
  if (JvmtiExport::can_force_early_return()) {
    Label Lno_early_ret;
    l(Rthr_state_addr, in_bytes(JavaThread::jvmti_thread_state_offset()), R16_thread);
    cmpi(CCR0, Rthr_state_addr, 0);
...
void InterpreterMacroAssembler::load_earlyret_value(TosState state, Register Rscratch1) {
  ...
  l(RjvmtiState, in_bytes(JavaThread::jvmti_thread_state_offset()), R16_thread);
  li(Rscratch2, 0);

  switch (state) {
    case atos: l(R17_tos, in_bytes(JvmtiThreadState::earlyret_oop_offset()), RjvmtiState);
               st(Rscratch2, in_bytes(JvmtiThreadState::earlyret_oop_offset()), RjvmtiState);
               break;
    case ltos:
#ifdef PPC64
               l(R17_tos, in_bytes(JvmtiThreadState::earlyret_value_offset()), RjvmtiState);
#else
               l(R30_tos2, in_bytes(JvmtiThreadState::earlyret_value_offset()), RjvmtiState);
               l(R17_tos, in_bytes(JvmtiThreadState::earlyret_value_offset()) + wordSize, RjvmtiState);
#endif
               break;
    ...
    case dtos: lfd(F15_ftos, in_bytes(JvmtiThreadState::earlyret_value_offset()), RjvmtiState);
               break;
#else
    case ftos: lwz(R17_tos, in_bytes(JvmtiThreadState::earlyret_value_offset()), RjvmtiState);
               break;
    case dtos: lwz(R0_SCRATCH, in_bytes(JvmtiThreadState::earlyret_value_offset()), RjvmtiState);
               lwz(R17_tos, in_bytes(JvmtiThreadState::earlyret_value_offset()) + wordSize, RjvmtiState);
               evmergelo(R17_tos, R0_SCRATCH, R17_tos);
               break;
#endif
    case vtos: break;
  ...
  st(Rscratch2, in_bytes(JvmtiThreadState::earlyret_value_offset()), RjvmtiState);
#ifndef PPC64
  st(Rscratch2, in_bytes(JvmtiThreadState::earlyret_value_offset()) + wordSize, RjvmtiState);
#endif
...
void InterpreterMacroAssembler::dispatch_Lbyte_code(TosState state, Register bytecode, address* table, bool verify) {
  ...
  sli(R12_scratch2, bytecode, LogBytesPerWord);
  lx(R11_scratch1, R11_scratch1, R12_scratch2);
...
void InterpreterMacroAssembler::load_receiver(Register Rparam_count, Register Rrecv_dst) {
  sli(Rrecv_dst, Rparam_count, Interpreter::logStackElementSize);
  lx(Rrecv_dst, Rrecv_dst, R15_esp);
}
...
void InterpreterMacroAssembler::pop_ptr(Register r) {
  lu(r, Interpreter::stackElementSize, R15_esp);
}
...
#else
void InterpreterMacroAssembler::pop_l(Register r, Register r2) {
  lwz(r2, Interpreter::stackElementSize, R15_esp);
  lwz(r, 2 * Interpreter::stackElementSize, R15_esp);
  addi(R15_esp, R15_esp, 2 * Interpreter::stackElementSize);
}
#endif
...
#else
void InterpreterMacroAssembler::pop_f(Register r) {
  lwzu(r, Interpreter::stackElementSize, R15_esp);
}
void InterpreterMacroAssembler::pop_d(Register r) {
  assert_different_registers(r, R0_SCRATCH);
  lwz(R0_SCRATCH, Interpreter::stackElementSize, R15_esp);
  lwzu(r, 2 * Interpreter::stackElementSize, R15_esp);
  evmergelo(r, R0_SCRATCH, r);
}
#endif
...
#else
void InterpreterMacroAssembler::push_l(Register r, Register r2) {
  stw(r, 0, R15_esp);
  stw(r2, - Interpreter::stackElementSize, R15_esp);
  addi(R15_esp, R15_esp, - 2 * Interpreter::stackElementSize );
}
#endif
...
#else
void InterpreterMacroAssembler::push_f(Register r) {
  stw(r, 0, R15_esp);
  addi(R15_esp, R15_esp, - Interpreter::stackElementSize );
}

void InterpreterMacroAssembler::push_d(Register r) {
  evstdd_unaligned(r, - Interpreter::stackElementSize, R15_esp, R0_SCRATCH);
  addi(R15_esp, R15_esp, - 2 * Interpreter::stackElementSize );
}
#endif
...
void InterpreterMacroAssembler::push_2ptrs(Register first, Register second) {
  st(first, 0, R15_esp);
  st(second, -Interpreter::stackElementSize, R15_esp);
  addi(R15_esp, R15_esp, - 2 * Interpreter::stackElementSize );
}
...
void InterpreterMacroAssembler::push_d_pop_l(FloatRegister d, Register lng) {
  stfd(d, 0, R15_esp);
  ld(lng, 0, R15_esp);
}
#else
#ifndef USE_SPE
void InterpreterMacroAssembler::push_l_pop_d(Register l, Register l2, FloatRegister d) {
  stw(l, 0, R15_esp);
  stw(l2, - Interpreter::stackElementSize, R15_esp);
  lfd(d, - Interpreter::stackElementSize, R15_esp);
}
void InterpreterMacroAssembler::push_d_pop_l(FloatRegister d, Register lng, Register lng2) {
  stfd(d, - Interpreter::stackElementSize, R15_esp);
  lwz(lng2, - Interpreter::stackElementSize, R15_esp);
  lwz(lng, 0, R15_esp);
}
#endif
#endif
...
void InterpreterMacroAssembler::get_cache_and_index_at_bcp(Register cache, int bcp_offset, size_t index_size) {
  ...
  sli(cache, cache, exact_log2(in_words(ConstantPoolCacheEntry::size()) * BytesPerWord));
...
void InterpreterMacroAssembler::load_resolved_reference_at_index(Register result, Register index) {
  ...
  sli(tmp, index, LogBytesPerHeapOop);
  // Load pointer for resolved_references[] objArray.
  l(result, ConstantPool::resolved_references_offset_in_bytes(), result);
  // JNIHandles::resolve(result)
  l(result, 0, result);
#ifdef ASSERT
  Label index_ok;
  lwa(R0, arrayOopDesc::length_offset_in_bytes(), result);
  sli(R0, R0, LogBytesPerHeapOop);
  cmp(CCR0, tmp, R0);
  blt(CCR0, index_ok);
...
void InterpreterMacroAssembler::generate_stack_overflow_check_with_compare_and_throw(Register Rmem_frame_size, Register Rscratch1) {
  ...
  l(Rscratch1, thread_(stack_overflow_limit));
  cmpl(CCR0/*is_stack_overflow*/, Rmem_frame_size, Rscratch1);
...
#ifdef ASSERT
  l(Rscratch1, 0, R1_SP);
  l(R0, 0, R21_sender_SP);
  cmp(CCR0, R0, Rscratch1);
...
void InterpreterMacroAssembler::index_check_without_pop(Register Rarray, Register Rindex, int index_shift, Register Rtmp, Register Rres) {
  ...
  rlicl(RsxtIndex, Rindex, 0, 32); // zero extend 32 bit -> 64 bit

  // Index check
  lwz(Rlength, arrayOopDesc::length_offset_in_bytes(), Rarray);
  cmplw(CCR0, Rindex, Rlength);
  sli(RsxtIndex, RsxtIndex, index_shift);
  blt(CCR0, LnotOOR);
...
void InterpreterMacroAssembler::get_const(Register Rdst) {
  l(Rdst, in_bytes(Method::const_offset()), R19_method);
}

void InterpreterMacroAssembler::get_constant_pool(Register Rdst) {
  get_const(Rdst);
  l(Rdst, in_bytes(ConstMethod::constants_offset()), Rdst);
}

void InterpreterMacroAssembler::get_constant_pool_cache(Register Rdst) {
  get_constant_pool(Rdst);
  l(Rdst, ConstantPool::cache_offset_in_bytes(), Rdst);
}

void InterpreterMacroAssembler::get_cpool_and_tags(Register Rcpool, Register Rtags) {
  get_constant_pool(Rcpool);
  l(Rtags, ConstantPool::tags_offset_in_bytes(), Rcpool);
}
...
void InterpreterMacroAssembler::unlock_if_synchronized_method(TosState state,
  ...
  Label Lunlocked, Lno_unlock;
  {
    ...
    rlicl_(R0, Raccess_flags, 64-JVM_ACC_SYNCHRONIZED_BIT, 63); // Extract bit and compare to 0.
    beq(CCR0, Lunlocked);
  ...
  bind(Lunlocked);
  {
    ...
    // Set up search loop: Calc num of iterations.
    {
      ...
      sri(Riterations, Riterations, exact_log2(delta));
      mtctr(Riterations);
    }
    ...
    // The search loop: Look for locked monitors.
    {
      ...
      l(Rcurrent_obj, 0, Rcurrent_obj_addr);
      addi(Rcurrent_obj_addr, Rcurrent_obj_addr, -delta);
      bind(Lloop);

      // Check if current entry is used.
      cmpi(CCR0, Rcurrent_obj, 0);
      bne(CCR0, Lexception);
...
void InterpreterMacroAssembler::merge_frames(Register Rsender_sp, Register return_pc, Register Rscratch1, Register Rscratch2) {
  ...
  l(Rscratch1, 0, R1_SP); // *SP
  l(Rsender_sp, _ijava_state_neg(sender_sp), Rscratch1); // top_frame_sp
  l(Rscratch2, 0, Rscratch1); // **SP
#ifdef ASSERT
  {
    Label Lok;
    l(R0, _ijava_state_neg(ijava_reserved), Rscratch1);
    cmpi(CCR0, R0, 0x5afe);
    beq(CCR0, Lok);
...
#endif
  if (return_pc!=noreg) {
    l(return_pc, _abi(lr), Rscratch1); // LR
  }

  // Merge top frames.
  subf(Rscratch1, R1_SP, Rsender_sp); // top_frame_sp - SP
  stux(Rscratch2, R1_SP, Rscratch1); // atomically set *(SP = top_frame_sp) = **SP
}

void InterpreterMacroAssembler::narrow(Register result) {
  Register ret_type = R11_scratch1;
  l(R11_scratch1, in_bytes(Method::const_offset()), R19_method);
...
void InterpreterMacroAssembler::lock_object(Register monitor, Register object) {
  if (UseHeavyMonitors) {
    ...
    l(displaced_header, oopDesc::mark_offset_in_bytes(), object);
    ...
    st(displaced_header, BasicObjectLock::lock_offset_in_bytes() +
    ...
    bne(CCR0, slow_case);
    release();
    st(R0/*==0!*/, BasicObjectLock::lock_offset_in_bytes() +
        BasicLock::displaced_header_offset_in_bytes(), monitor);
    b(done);
...
void InterpreterMacroAssembler::unlock_object(Register monitor, bool check_for_exceptions) {
  if (UseHeavyMonitors) {
    ...
    if (UseBiasedLocking) {
      ...
      l(object, BasicObjectLock::obj_offset_in_bytes(), monitor);
      assert(oopDesc::mark_offset_in_bytes() == 0, "offset of _mark is not 0");
      biased_locking_exit(CCR0, object, displaced_header, free_slot);
    }

    // Test first if we are in the fast recursive case.
    l(displaced_header, BasicObjectLock::lock_offset_in_bytes() +
           BasicLock::displaced_header_offset_in_bytes(), monitor);

    // If the displaced header is zero, we have a recursive unlock.
    cmpi(CCR0, displaced_header, 0);
    ...
    // The object address from the monitor is in object.
    if (!UseBiasedLocking) { l(object, BasicObjectLock::obj_offset_in_bytes(), monitor); }
    addi(object_mark_addr, object, oopDesc::mark_offset_in_bytes());
    ...
    bind(free_slot);
    li(R0, 0);
    st(R0, BasicObjectLock::obj_offset_in_bytes(), monitor);
    bind(done);
  }
}
...
void InterpreterMacroAssembler::call_from_interpreter(Register Rtarget_method, Register Rret_addr, Register Rscratch1, Register Rscratch2) {
  ...
  l(Rtarget_addr, in_bytes(Method::from_interpreted_offset()), Rtarget_method);
  ...
  if (JvmtiExport::can_post_interpreter_events()) {
    ...
    beq(CCR0, done);
    l(Rtarget_addr, in_bytes(Method::interpreter_entry_offset()), Rtarget_method);
    align(32, 12);
    bind(done);
  }
  ...
  clrri(Rscratch2, Rscratch2, exact_log2(frame::alignment_in_bytes)); // round towards smaller address
...
#ifdef ASSERT
  l(Rscratch1, _ijava_state_neg(top_frame_sp), Rscratch2); // Rscratch2 contains fp
  cmp(CCR0, R21_sender_SP, Rscratch1);
...
void InterpreterMacroAssembler::set_method_data_pointer_for_bcp() {
  ...
  Label get_continue;
  l(R28_mdx, in_bytes(Method::method_data_offset()), R19_method);
...
void InterpreterMacroAssembler::test_invocation_counter_for_mdp(Register invocation_count,
  ...
  cmp(CCR1, Rscratch /* InterpreterProfileLimit */, invocation_count);
  bne(CCR0, done);
...
void InterpreterMacroAssembler::test_backedge_count_for_osr(Register backedge_count, Register method_counters,
  ...
  l(R11_scratch1, nmethod::osr_entry_point_offset(), osr_nmethod);
  mtlr(R0);
  mtctr(R11_scratch1);
  bctr();
...
void InterpreterMacroAssembler::increment_mdp_data_at(Register counter_addr,
  ...
  // Load the counter.
  l(Rbumped_count, 0, counter_addr);
  ...
  if (decrement) {
    ...
    st(Rbumped_count, 0, counter_addr);
...
void InterpreterMacroAssembler::test_mdp_data_at(int offset,
  ...
  l(test_out, offset, R28_mdx);
  cmp(CCR0,  value, test_out);
...
void InterpreterMacroAssembler::update_mdp_by_offset(int offset_of_disp,
  ...
  l(scratch, offset_of_disp, R28_mdx);
...
void InterpreterMacroAssembler::update_mdp_by_offset(Register reg,
  ...
  l(scratch, offset_of_disp, scratch);
  add(R28_mdx, scratch, R28_mdx);
}
...
void InterpreterMacroAssembler::profile_switch_case(Register index,
  ...
  if (ProfileInterpreter) {
    ...
    assert (in_bytes(MultiBranchData::per_case_size()) == PPC64_ONLY(16) NOT_PPC64(8), "so that shladd works");
    sli(scratch1, index, exact_log2(in_bytes(MultiBranchData::per_case_size())));
...
void InterpreterMacroAssembler::profile_obj_type(Register obj, Register mdo_addr_base,
  ...
  l(tmp, mdo_addr_offs, mdo_addr_base);
  ...
  clrri(R0, tmp, exact_log2(-TypeEntries::type_klass_mask));
  ...
  clrri_(R0, tmp, exact_log2(-TypeEntries::type_mask));
  ...
  bind(do_update);
  // update profile
  st(R0, mdo_addr_offs, mdo_addr_base);
    ...
    if (MethodData::profile_arguments()) {
      ...
      for (int i = 0; i < TypeProfileArgsLimit; i++) {
        if (i > 0 || MethodData::profile_return()) {
          ...
          l(tmp1, in_bytes(TypeEntriesAtCall::cell_count_offset())-off_to_args, R28_mdx);
          cmpi(CCR0, tmp1, (i+1)*TypeStackSlotEntries::per_arg_count());
          addi(tmp1, tmp1, -i*TypeStackSlotEntries::per_arg_count());
          blt(CCR0, done);
        }
        l(tmp1, in_bytes(Method::const_offset()), callee);
        ...
        l(tmp2, in_bytes(TypeEntriesAtCall::stack_slot_offset(i))-off_to_args, R28_mdx);
        subf(tmp1, tmp2, tmp1);

        sli(tmp1, tmp1, Interpreter::logStackElementSize);
        lx(tmp1, tmp1, R15_esp);
      ...
      if (MethodData::profile_return()) {
        ...
        sli(tmp1, tmp1, exact_log2(DataLayout::cell_size));
...
void InterpreterMacroAssembler::profile_parameters_type(Register tmp1, Register tmp2, Register tmp3, Register tmp4) {
  if (ProfileInterpreter && MethodData::profile_parameters()) {
    ...
    l(entry_offset, in_bytes(ArrayData::array_len_offset()), mdp);
    ...
    sli(entry_offset, entry_offset, exact_log2(DataLayout::cell_size));
    ...
    l(tmp3, entry_offset, mdp);
    sli(tmp3, tmp3, Interpreter::logStackElementSize);
    neg(tmp3, tmp3);
    // Read the parameter from the local area.
    lx(tmp3, tmp3, R18_locals);
    ...
    cmpi(CCR0, entry_offset, off_base + delta);
...
void InterpreterMacroAssembler::add_monitor_to_stack(bool stack_is_empty, Register Rtemp1, Register Rtemp2) {
  ...
  st(R1_SP, _ijava_state_neg(top_frame_sp), esp); // esp contains fp
  ...
  if (!stack_is_empty) {
    ...
    sri_(n_slots, n_slots, Interpreter::logStackElementSize);          // Compute number of slots to copy.
    ...
    // loop
    bind(copy_slot);
    l(slot, 0, esp);              // Move expression stack down.
    st(slot, -monitor_size, esp); // distance = monitor_size
    addi(esp, esp, Interpreter::stackElementSize);
    bdnz(copy_slot);
...
void InterpreterMacroAssembler::load_local_int(Register Rdst_value, Register Rdst_address, Register Rindex) {
  sli(Rdst_address, Rindex, Interpreter::logStackElementSize);
...
#else
void InterpreterMacroAssembler::load_local_long(Register Rdst_value, Register Rdst2_value, Register Rdst_address, Register Rindex) {
  sli(Rdst_address, Rindex, Interpreter::logStackElementSize);
  subf(Rdst_address, Rdst_address, R18_locals);
  lwz(Rdst2_value, -4, Rdst_address);
  lwz(Rdst_value, 0, Rdst_address);
}
#endif
...
void InterpreterMacroAssembler::load_local_double(FloatRegister Rdst_value, Register Rdst_address, Register Rindex) {
  sli(Rdst_address, Rindex, Interpreter::logStackElementSize);
  subf(Rdst_address, Rdst_address, R18_locals);
  lfd(Rdst_value, -Interpreter::stackElementSize, Rdst_address);
}
#else
// Load a local variable at index in Rindex into register Rdst_value.
// Also puts address of local into Rdst_address as a service.
...
//   - Rdst_value
//   - Rdst_address
void InterpreterMacroAssembler::load_local_float(Register Rdst_value, Register Rdst_address, Register Rindex) {
  sli(Rdst_address, Rindex, Interpreter::logStackElementSize);
  subf(Rdst_address, Rdst_address, R18_locals);
  lwz(Rdst_value, 0, Rdst_address);
}
...
//   - Rdst_value
//   - Rdst_address
void InterpreterMacroAssembler::load_local_double(Register Rdst_value, Register Rdst_address, Register Rindex) {
  assert_different_registers(R12_scratch2, Rdst_address);
  assert_different_registers(R12_scratch2, Rdst_value);
  sli(Rdst_address, Rindex, Interpreter::logStackElementSize);
  subf(Rdst_address, Rdst_address, R18_locals);
  lwz(R12_scratch2, - Interpreter::stackElementSize, Rdst_address);
  lwz(Rdst_value, 0, Rdst_address);
  evmergelo(Rdst_value, R12_scratch2, Rdst_value);
}
#endif
...
void InterpreterMacroAssembler::store_local_int(Register Rvalue, Register Rindex) {
  sli(Rindex, Rindex, Interpreter::logStackElementSize);
...
#else
void InterpreterMacroAssembler::store_local_long(Register Rvalue, Register Rvalue2, Register Rindex) {
  sli(Rindex, Rindex, Interpreter::logStackElementSize);
  subf(Rindex, Rindex, R18_locals);
  stw(Rvalue2, -4, Rindex);
  stw(Rvalue, 0, Rindex);
}
#endif
...
void InterpreterMacroAssembler::store_local_double(FloatRegister Rvalue, Register Rindex) {
  sli(Rindex, Rindex, Interpreter::logStackElementSize);
  subf(Rindex, Rindex, R18_locals);
  stfd(Rvalue, -Interpreter::stackElementSize, Rindex);
}
#else
// Store an int value at local variable slot Rindex.
// Kills:
//   - Rindex
void InterpreterMacroAssembler::store_local_float(Register Rvalue, Register Rindex) {
  sli(Rindex, Rindex, Interpreter::logStackElementSize);
  subf(Rindex, Rindex, R18_locals);
  stw(Rvalue, 0, Rindex);
}
...
// Kills:
//   - Rindex
void InterpreterMacroAssembler::store_local_double(Register Rvalue, Register Rindex) {
  assert_different_registers(R12_scratch2, Rindex);
  sli(Rindex, Rindex, Interpreter::logStackElementSize);
  subf(Rindex, Rindex, R18_locals);
  stw(Rvalue, 0, Rindex);
  evmergehi(R12_scratch2, Rvalue, Rvalue);
  stw(R12_scratch2, - Interpreter::stackElementSize, Rindex);
}
#endif
...
void InterpreterMacroAssembler::check_and_forward_exception(Register Rscratch1, Register Rscratch2) {
  ...
  l(Rexception, thread_(pending_exception));
  cmpi(CCR0, Rexception, 0);
  beq(CCR0, Ldone);
  li(Rtmp, 0);
  mr_if_needed(R3, Rexception);
  st(Rtmp, thread_(pending_exception)); // Clear exception in thread
...
void InterpreterMacroAssembler::save_interpreter_state(Register scratch) {
  l(scratch, 0, R1_SP);
  st(R15_esp, _ijava_state_neg(esp), scratch);
  st(R14_bcp, _ijava_state_neg(bcp), scratch);
  st(R26_monitor, _ijava_state_neg(monitors), scratch);
  if (ProfileInterpreter) { st(R28_mdx, _ijava_state_neg(mdx), scratch); }
...
void InterpreterMacroAssembler::restore_interpreter_state(Register scratch, bool bcp_and_mdx_only) {
  l(scratch, 0, R1_SP);
  l(R14_bcp, _ijava_state_neg(bcp), scratch); // Changed by VM code (exception).
  if (ProfileInterpreter) { l(R28_mdx, _ijava_state_neg(mdx), scratch); } // Changed by VM code.
  if (!bcp_and_mdx_only) {
    // Following ones are Metadata.
    l(R19_method, _ijava_state_neg(method), scratch);
    l(R27_constPoolCache, _ijava_state_neg(cpoolCache), scratch);
    // Following ones are stack addresses and don't require reload.
    l(R15_esp, _ijava_state_neg(esp), scratch);
    l(R18_locals, _ijava_state_neg(locals), scratch);
    l(R26_monitor, _ijava_state_neg(monitors), scratch);
  }
#ifdef ASSERT
  {
    Label Lok;
    subf(R0, R1_SP, scratch);
    cmpi(CCR0, R0, frame::abi_reg_args_size + frame::ijava_state_size);
    bge(CCR0, Lok);
  ...
  {
    Label Lok;
    l(R0, _ijava_state_neg(ijava_reserved), scratch);
    cmpi(CCR0, R0, 0x5afe);
    beq(CCR0, Lok);
...
void InterpreterMacroAssembler::get_method_counters(Register method,
  ...
  Label has_counters;
  l(Rcounters, in_bytes(Method::method_counters_offset()), method);
  cmpi(CCR0, Rcounters, 0);
...
void InterpreterMacroAssembler::verify_oop_or_return_address(Register reg, Register Rtmp) {
  ...
  sri_(Rtmp, reg, log2_bytecode_size_limit);
  bne(CCR0, test);
...
#else
  save_LR(Rtmp); // Save in old frame.
#endif
...
void InterpreterMacroAssembler::push_interpreter_frame(Register top_frame_size, Register parent_frame_resize,
  ...
  l(tmp1, _top_ijava_frame_abi(frame_manager_lr), R1_SP);
  mr(tmp2/*top_frame_sp*/, R1_SP);
  // Move initial_caller_sp.
  l(tmp4, _top_ijava_frame_abi(initial_caller_sp), R1_SP);
  ...
  st(tmp1, _abi(lr), R1_SP);
  // Set top_frame_sp info for new parent frame.
  st(tmp2, _parent_ijava_frame_abi(top_frame_sp), R1_SP);
  st(tmp4, _parent_ijava_frame_abi(initial_caller_sp), R1_SP);

  // Push new TOP_IJAVA_FRAME.
  push_frame(top_frame_size, tmp2);

  get_PC_trash_LR(tmp3);
  st(tmp3, _top_ijava_frame_abi(frame_manager_lr), R1_SP);
  // Used for non-initial callers by unextended_sp().
  st(R1_SP, _top_ijava_frame_abi(initial_caller_sp), R1_SP);
}
...
void InterpreterMacroAssembler::pop_interpreter_frame(Register tmp1, Register tmp2, Register tmp3, Register tmp4) {
  ...
  l(tmp1/*caller's sp*/, _abi(callers_sp), R1_SP);
  l(tmp3, _abi(lr), tmp1);

  l(tmp4, _parent_ijava_frame_abi(initial_caller_sp), tmp1);

  l(tmp2/*caller's caller's sp*/, _abi(callers_sp), tmp1);
  // Merge top frame.
  st(tmp2, _abi(callers_sp), R1_SP);

  l(tmp2, _parent_ijava_frame_abi(top_frame_sp), tmp1);
  ...
  st(tmp4, _top_ijava_frame_abi(initial_caller_sp), R1_SP);

  // Store the top-frame stack-pointer for c2i adapters.
  st(R1_SP, _top_ijava_frame_abi(top_frame_sp), R1_SP);
}
...
void InterpreterMacroAssembler::pop_interpreter_frame_to_state(Register state, Register tmp1, Register tmp2, Register tmp3) {
  ...
  if (state == R14_state) {
    l(tmp1/*state's fp*/, state_(_last_Java_fp));
    l(tmp2/*state's sp*/, state_(_last_Java_sp));
  } else if (state == R15_prev_state) {
    l(tmp1/*state's fp*/, prev_state_(_last_Java_fp));
    l(tmp2/*state's sp*/, prev_state_(_last_Java_sp));
  } else {
    ShouldNotReachHere();
  }

  // Merge top frames.
  st(tmp1, _abi(callers_sp), R1_SP);
...
void InterpreterMacroAssembler::resize_frame_to_initial_caller(Register tmp1, Register tmp2) {
  l(tmp1, _parent_ijava_frame_abi(initial_caller_sp), R1_SP);
  l(tmp2, _parent_ijava_frame_abi(callers_sp), R1_SP);
  st(tmp2, _parent_ijava_frame_abi(callers_sp), tmp1); // Fix back chain ...
