...
void C1_MacroAssembler::build_frame(int frame_size_in_bytes, int bang_size_in_bytes) {
  ...
  mflr(return_pc);
  // Get callers sp.
  st(return_pc, _abi(lr), R1_SP);           // SP->lr = return_pc
...
void C1_MacroAssembler::lock_object(Register Rmark, Register Roop, Register Rbox, Register Rscratch, Label& slow_case) {
  ...
  // Load object header.
  l(Rmark, oopDesc::mark_offset_in_bytes(), Roop);

  verify_oop(Roop);

  // Save object being locked into the BasicObjectLock...
  st(Roop, BasicObjectLock::obj_offset_in_bytes(), Rbox);
  ...
  st(Rmark, BasicLock::displaced_header_offset_in_bytes(), Rbox);
  ...
  st(R0/*==0, perhaps*/, BasicLock::displaced_header_offset_in_bytes(), Rbox);
  bne(CCR0, slow_int);
...
void C1_MacroAssembler::unlock_object(Register Rmark, Register Roop, Register Rbox, Label& slow_case) {
  ...
  if (UseBiasedLocking) {
    ...
    l(Roop, BasicObjectLock::obj_offset_in_bytes(), Rbox);
    verify_oop(Roop);
    biased_locking_exit(CCR0, Roop, R0, done);
  }
  // Test first it it is a fast recursive unlock.
  l(Rmark, BasicLock::displaced_header_offset_in_bytes(), Rbox);
  cmpi(CCR0, Rmark, 0);
  beq(CCR0, done);
  if (!UseBiasedLocking) {
    // Load object.
...
void C1_MacroAssembler::initialize_header(Register obj, Register klass, Register len, Register t1, Register t2) {
  ...
  if (UseBiasedLocking && !len->is_valid()) {
    l(t1, in_bytes(Klass::prototype_header_offset()), klass);
  } else {
    load_const_optimized(t1, (intx)markOopDesc::prototype());
  }
  st(t1, oopDesc::mark_offset_in_bytes(), obj);
...
void C1_MacroAssembler::initialize_body(Register base, Register index) {
  ...
  sri(index, index, LogBytesPerWord);
  clear_memory_word(base, index);
}
...
void C1_MacroAssembler::initialize_body(Register obj, Register tmp1, Register tmp2,
  ...
  if (index <= 6) {
    ...
    if (index > 0) { li(tmp, 0); }
    for (int i = 0; i < index; ++i) { st(tmp, hdr_size_in_bytes + i * HeapWordSize, obj); }

  } else if (index < (2<<cl_w_addr_bits)-1) {
    // simple loop
    Label loop;
  ...
  bind(loop);
    ...
    // like clear_memory_word
    Label startloop, fast, fastloop, restloop, done;

    addi(base_ptr, obj, hdr_size_in_bytes);           // Compute address of first element.
    load_const_optimized(cnt_words, index);
...
void C1_MacroAssembler::null_check(Register r, Label* Lnull) {
  if (TrapBasedNullChecks) { // SIGTRAP based
    trap_null_check(r);
  } else if (GenerateCompilerNullChecks && Lnull == NULL) {
    warning("!TrapBasedNullChecks && GenerateCompilerNullChecks option settings concidered unstable");
    // Assume this called from share/vm/c1/c1_LIRAssembler.cpp, lir_null_check only.
    // R12 made temp only for that node (c1_LinearScan_ppc.hpp, pd_add_temps).
    // If it's another node, it's a bug!
    l(R12, r);
  } else { // explicit
