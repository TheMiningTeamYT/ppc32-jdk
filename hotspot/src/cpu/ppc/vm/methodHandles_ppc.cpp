...
void MethodHandles::load_klass_from_Class(MacroAssembler* _masm, Register klass_reg, Register temp_reg, Register temp2_reg) {
  ...
  __ l(klass_reg, java_lang_Class::klass_offset_in_bytes(), klass_reg);
}
...
void MethodHandles::verify_klass(MacroAssembler* _masm,
  ...
  __ cmpi(CCR0, obj_reg, 0);
  __ beq(CCR0, L_bad);
  __ load_klass(temp_reg, obj_reg);
  __ load_const_optimized(temp2_reg, (address) klass_addr);
  __ l(temp2_reg, 0, temp2_reg);
  __ cmp(CCR0, temp_reg, temp2_reg);
  __ beq(CCR0, L_ok);
  __ l(temp_reg, klass->super_check_offset(), temp_reg);
  __ cmp(CCR0, temp_reg, temp2_reg);
  __ beq(CCR0, L_ok);
  __ BIND(L_bad);
...
void MethodHandles::jump_from_method_handle(MacroAssembler* _masm, Register method, Register target, Register temp,
  ...
  if (!for_compiler_entry && JvmtiExport::can_post_interpreter_events()) {
    ...
    __ l(target, in_bytes(Method::interpreter_entry_offset()), R19_method);
    __ mtctr(target);
    __ bctr();
  ...
  __ l(target, in_bytes(entry_offset), R19_method);
  __ mtctr(target);
  __ bctr();
...
void MethodHandles::jump_to_lambda_form(MacroAssembler* _masm,
  ...
  __ l(method_temp, NONZERO(java_lang_invoke_MemberName::vmtarget_offset_in_bytes()), method_temp);

  if (VerifyMethodHandles && !for_compiler_entry) {
    // make sure recv is already on stack
    __ l(temp2, in_bytes(Method::const_offset()), method_temp);
    ...
    Label L;
    __ l(temp2, __ argument_offset(temp2, temp2, 0), CC_INTERP_ONLY(R17_tos) NOT_CC_INTERP(R15_esp));
    __ cmp(CCR1, temp2, recv);
    __ beq(CCR1, L);
...
address MethodHandles::generate_method_handle_interpreter_entry(MacroAssembler* _masm,
  ...
  if (VerifyMethodHandles) {
    assert(Method::intrinsic_id_size_in_bytes() == 1, "assuming Method::_intrinsic_id is u1");
  ...
  if (ref_kind == 0 || MethodHandles::ref_kind_has_receiver(ref_kind)) {
    __ l(param_size, in_bytes(Method::const_offset()), R19_method);
  ...
  if (!is_signature_polymorphic_static(iid)) {
    __ l(tmp_mh = temp1, __ argument_offset(param_size, param_size, 0), argbase);
    DEBUG_ONLY(param_size = noreg);
  }
  ...
  if (iid == vmIntrinsics::_invokeBasic) {
    ...
    if (MethodHandles::ref_kind_has_receiver(ref_kind)) {
      ...
      __ l(tmp_recv = temp1, __ argument_offset(param_size, param_size, 0), argbase);
      DEBUG_ONLY(param_size = noreg);
    }
    Register R19_member = R19_method;  // MemberName ptr; incoming method ptr is dead now
    __ l(R19_member, RegisterOrConstant((intptr_t)Interpreter::stackElementSize), argbase);
...
void MethodHandles::generate_method_handle_dispatch(MacroAssembler* _masm,
  ...
  if (iid == vmIntrinsics::_invokeBasic) {
    ...
    case vmIntrinsics::_linkToSpecial:
      ...
      __ l(R19_method, NONZERO(java_lang_invoke_MemberName::vmtarget_offset_in_bytes()), member_reg);
      break;
    ...
    case vmIntrinsics::_linkToVirtual:
    {
      ...
      __ l(temp2_index, NONZERO(java_lang_invoke_MemberName::vmindex_offset_in_bytes()), member_reg);

      if (VerifyMethodHandles) {
        Label L_index_ok;
        __ cmpi(CCR1, temp2_index, 0);
    ...
    case vmIntrinsics::_linkToInterface:
    {
      ...
      __ l(vtable_index, NONZERO(java_lang_invoke_MemberName::vmindex_offset_in_bytes()), member_reg);
      if (VerifyMethodHandles) {
        Label L_index_ok;
        __ cmpi(CCR1, vtable_index, 0);
        __ bge(CCR1, L_index_ok);
...
void trace_method_handle_stub(const char* adaptername,
  ...
  if (Verbose) {
    ...
    const int abi_offset = frame::abi_reg_args_size / BytesPerWord;
