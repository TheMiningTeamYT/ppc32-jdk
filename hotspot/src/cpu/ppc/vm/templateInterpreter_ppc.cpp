...
address TemplateInterpreterGenerator::generate_return_entry_for(TosState state, int step, size_t index_size) {
  ...
  switch (state) {
    case ltos: NOT_PPC64( __ mr(R30_tos2, R3_RET); __ mr(R17_tos, R4_RET2); break);
    case btos:
    case ztos:
    case ctos:
    case stos:
    ...
    case ftos: __ mr(R17_tos, R3_RET); break;
    case dtos: __ evmergehilo(R17_tos, R3_RET, R3_RET); break;
#else // SPE_ABI and not USE_SPE
    case ftos: __ stw(R3_RET, 0, R15_esp);
               __ lfs(F15_ftos, 0, R15_esp);
               break;
    case dtos: __ stw(R3_RET, -4, R15_esp);
  ...
  __ l(R12_scratch2, _ijava_state_neg(top_frame_sp), R11_scratch1);
...
#else
  __ lbz(size, in_bytes(ConstantPoolCache::base_offset() + ConstantPoolCacheEntry::flags_offset()) + PPC64_ONLY(7) NOT_PPC64(3), cache);
#endif
  __ sli(size, size, Interpreter::logStackElementSize);
...
#else
    case dtos:
      __ evmergehilo(R17_tos, R3_RET, R3_RET);
     break;
#endif
...
void TemplateInterpreterGenerator::lock_method(Register Rflags, Register Rscratch1, Register Rscratch2, bool flags_preloaded) {
  ...
  // Get synchronization object to Rscratch2.
  {
    ...
    __ l(Robj_to_lock, R18_locals);
    __ b(Ldone);

    __ bind(Lstatic); // Static case: Lock the java mirror
    __ l(Robj_to_lock, in_bytes(Method::const_offset()), R19_method);
    __ l(Robj_to_lock, in_bytes(ConstMethod::constants_offset()), Robj_to_lock);
    __ l(Robj_to_lock, ConstantPool::pool_holder_offset_in_bytes(), Robj_to_lock);
    __ l(Robj_to_lock, mirror_offset, Robj_to_lock);
  ...
  __ st(Robj_to_lock, BasicObjectLock::obj_offset_in_bytes(), R26_monitor);
...
void TemplateInterpreterGenerator::generate_fixed_frame(bool native_call, Register Rsize_of_parameters, Register Rsize_of_locals) {
  ...
  __ l(Rconst_method, method_(const));
  ...
  if (native_call) {
...
#else
    // for PPC32 the minimum parameter save area size is 0, and is only increased if
    // args should be spilled on the stack. The below code calculates the upper bound of
    // necessary frame, since signature parsing is necessary to get exact result
    // The semantics of Rsize_of_parameters for PPC32 is "the size of parameters on stack"
    Label need_arg_spill;
    __ addic_ (top_frame_size, Rsize_of_parameters, -(Argument::n_register_parameters - 2));
    __ bge(CCR0, need_arg_spill);
    __ li (top_frame_size, 0);
    __ bind(need_arg_spill);
#endif
    __ sli(Rsize_of_parameters, Rsize_of_parameters, Interpreter::logStackElementSize);
    // PPC32: This +1 is a hack to double the amount of space allocated for parameters, this is
    // likely far more than needed as in the worst case when parameters have to be placed on the stack
    // they would be aligned as follows LONG | INT | EMPTY | LONG ...
    __ sli(top_frame_size, top_frame_size, Interpreter::logStackElementSize OPENJDK_PPC32_ONLY(+ 1));
    ...
    __ sli(Rsize_of_parameters, Rsize_of_parameters, Interpreter::logStackElementSize);
    __ sli(Rsize_of_locals, Rsize_of_locals, Interpreter::logStackElementSize);
    ...
    __ sub(parent_frame_resize, R1_SP, R15_esp); // <0, off by Interpreter::stackElementSize!
    __ sli(top_frame_size, top_frame_size, Interpreter::logStackElementSize);
  ...
  __ l(R27_constPoolCache, in_bytes(ConstMethod::constants_offset()), Rconst_method);
  __ l(R27_constPoolCache, ConstantPool::cache_offset_in_bytes(), R27_constPoolCache);

  // Set method data pointer.
  if (ProfileInterpreter) {
    Label zero_continue;
    __ l(R28_mdx, method_(method_data));
    __ cmpi(CCR0, R28_mdx, 0);
  ...
  __ st(R19_method, _ijava_state_neg(method), R1_SP);
  __ st(R21_sender_SP, _ijava_state_neg(sender_sp), R1_SP);
  __ st(R27_constPoolCache, _ijava_state_neg(cpoolCache), R1_SP);
  __ st(R18_locals, _ijava_state_neg(locals), R1_SP);
  ...
  if (native_call) {
    __ st(R26_monitor, _ijava_state_neg(monitors), R1_SP);
    __ st(R14_bcp, _ijava_state_neg(bcp), R1_SP);
    if (ProfileInterpreter) { __ st(R28_mdx, _ijava_state_neg(mdx), R1_SP); }
  }
#ifdef ASSERT
  else {
    __ st(R12_scratch2, _ijava_state_neg(monitors), R1_SP);
    __ st(R12_scratch2, _ijava_state_neg(bcp), R1_SP);
    __ st(R12_scratch2, _ijava_state_neg(mdx), R1_SP);
  }
  __ st(R11_scratch1, _ijava_state_neg(ijava_reserved), R1_SP);
  __ st(R12_scratch2, _ijava_state_neg(esp), R1_SP);
  __ st(R12_scratch2, _ijava_state_neg(lresult), R1_SP);
#ifndef PPC64
  __ st(R12_scratch2, _ijava_state_neg(lresult) + wordSize, R1_SP);
#endif // PPC64
#ifndef USE_SPE
  __ st(R12_scratch2, _ijava_state_neg(fresult), R1_SP);
#ifndef PPC64
  __ st(R12_scratch2, _ijava_state_neg(fresult) + wordSize, R1_SP);
#endif // PPC64
#endif // USE_SPE
#endif // ASSERT
  __ subf(R12_scratch2, top_frame_size, R1_SP);
  __ st(R0, _ijava_state_neg(oop_tmp), R1_SP);
  __ st(R12_scratch2, _ijava_state_neg(top_frame_sp), R1_SP);
...
#ifdef ASSERT
    __ l(R9_ARG7, 0, R1_SP);
    __ l(R10_ARG8, 0, R21_sender_SP);
    __ cmp(CCR0, R9_ARG7, R10_ARG8);
...
address TemplateInterpreterGenerator::generate_math_entry(AbstractInterpreter::MethodKind kind) {
  ...
  __ lwz(R11_scratch1, 2 * Interpreter::stackElementSize, R15_esp);
  __ evmergelo(R3_ARG1, R3_ARG1, R11_scratch1);
#endif
...
address TemplateInterpreterGenerator::generate_native_entry(bool synchronized) {
  ...
  if (synchronized) {
    ...
    __ st(R26_monitor, _ijava_state_neg(monitors), R11_scratch1);
  }
  ...
  __ l(signature_handler_fd, method_(signature_handler));
  Label call_signature_handler;

  __ cmpi(CCR0, signature_handler_fd, 0);
  ...
  __ l(signature_handler_fd, 0, signature_handler_fd);
#endif
  ...
  // If the method is static, get mirror into R4_ARG2.
  {
    ...
    __ l(R11_scratch1, in_bytes(ConstMethod::constants_offset()), R11_scratch1);
    // pool_holder = method->constants()->pool_holder();
    __ l(R11_scratch1/*pool_holder*/, ConstantPool::pool_holder_offset_in_bytes(),
          R11_scratch1/*constants*/);

    const int mirror_offset = in_bytes(Klass::java_mirror_offset());

    // mirror = pool_holder->klass_part()->java_mirror();
    __ l(R0/*mirror*/, mirror_offset, R11_scratch1/*pool_holder*/);
    // state->_native_mirror = mirror;

    __ l(R11_scratch1, 0, R1_SP);
    __ st(R0/*mirror*/, _ijava_state_neg(oop_tmp), R11_scratch1);
  ...
  __ l(native_method_fd, method_(native_function));
  ...
  __ li(R0, 0);
  __ l(R11_scratch1, 0, R1_SP);
  __ st(R3_RET, _ijava_state_neg(lresult), R11_scratch1);
#ifndef PPC64
  __ st(R4_RET2, _ijava_state_neg(lresult) + wordSize, R11_scratch1);
#endif
#ifndef USE_SPE
  __ stfd(F1_RET, _ijava_state_neg(fresult), R11_scratch1);
#endif
  __ st(R0/*mirror*/, _ijava_state_neg(oop_tmp), R11_scratch1); // reset
  ...
  //__ st(R11_scratch1, _top_ijava_frame_abi(frame_manager_lr), R1_SP);
  ...
  __ l(active_handles, thread_(active_handles));
  ...
  __ l(R0/*pending_exception*/, thread_(pending_exception));
  __ cmpi(CCR0, R0/*pending_exception*/, 0);
  ...
  __ l(R11_scratch1, 0, R1_SP);
  __ l(R3_RET, _ijava_state_neg(lresult), R11_scratch1);
#ifndef PPC64
  __ l(R4_RET2, _ijava_state_neg(lresult) + wordSize, R11_scratch1);
#endif
#ifndef USE_SPE
  ...
  __ l(return_pc, 0, R1_SP);
  __ l(return_pc, _abi(lr), return_pc);
  ...
  __ l(R3_ARG1/*exception*/, thread_(pending_exception));
  __ li(R4_ARG2, 0);
  __ st(R4_ARG2, thread_(pending_exception));
...
address TemplateInterpreterGenerator::generate_normal_entry(bool synchronized) {
  ...
  __ sri_(Rnum, Rnum, Interpreter::logStackElementSize);
...
int AbstractInterpreter::size_top_interpreter_activation(Method* method) {
  const int max_alignment_size = StackAlignmentInBytes / Interpreter::stackElementSize;
...
// given sections.
// This overestimates the stack by one (PPC64) or three (PPC32) slot in case of alignments.
int AbstractInterpreter::size_activation(int max_stack,
  ...
  // TODO `1' below seems to be required by layout_activation,
  // see TODO near sp calculation there
  const int size = 1                                         +
    max_stack                                                +
  ...
  return round_to(size, max_alignment_space);
}
...
void AbstractInterpreter::layout_activation(Method* method,
  ...
  // TODO check if sp calculation below is correct. esp_base is one below monitor,
  // so actual maximum of elements of stack below seems to be (max_stack + 1).
  // Reducing sp by one here and in size activation (see TODO there) still works.
  // Test it more carefully later.
...
void TemplateInterpreterGenerator::generate_throw_exception() {
  ...
  Interpreter::_remove_activation_preserving_args_entry = __ pc();
  {
    ...
    __ l(R4_ARG2, in_bytes(Method::const_offset()), R19_method);
    ...
    if (ProfileInterpreter) {
      __ set_method_data_pointer_for_bcp();
      __ l(R11_scratch1, 0, R1_SP);
      __ st(R28_mdx, _ijava_state_neg(mdx), R11_scratch1);
    }
#if INCLUDE_JVMTI
    Label L_done;
    ...
    __ cmpi(CCR0, R4_ARG2, 0);
    __ beq(CCR0, L_done);
    __ st(R4_ARG2, wordSize, R15_esp);
    __ bind(L_done);
#endif // INCLUDE_JVMTI
    __ dispatch_next(vtos);
  }
  ...
  Interpreter::_remove_activation_entry = __ pc();
  {
    ...
    __ verify_thread();
    __ verify_oop(Rexception);
    __ st(Rexception, in_bytes(JavaThread::vm_result_offset()), R16_thread);
...
address TemplateInterpreterGenerator::generate_earlyret_entry_for(TosState state) {
  ...
  __ l(Rscratch1, in_bytes(JavaThread::jvmti_thread_state_offset()), R16_thread);
...
#else
    case ltos: __ mr(R3_RET, R30_tos2); __ mr(R4_RET2, R17_tos); break;
...
#else
    case dtos: __ evmergehilo(R3_RET, R17_tos, R17_tos); break;
#endif
...
address TemplateInterpreterGenerator::generate_trace_code(TosState state) {
  ...
  case ltos:
    ...
    tsize = PPC64_ONLY(3) NOT_PPC64(2);
    break;
  case atos:
  ...
  if (TraceBytecodesAt > 0 && TraceBytecodesAt < max_intx) {
    ...
    __ l(R11_scratch1, offs1, R11_scratch1);
    __ lwa(R12_scratch2, offs2, R12_scratch2);
    __ cmp(CCR0, R12_scratch2, R11_scratch1);
    __ blt(CCR0, Lskip_vm_call);
  }

  __ push(state);
  // Load 2 topmost expression stack values.
  __ l(R6_ARG4, tsize*Interpreter::stackElementSize, R15_esp);
  __ l(R5_ARG3, Interpreter::stackElementSize, R15_esp);
  __ mflr(R31);
