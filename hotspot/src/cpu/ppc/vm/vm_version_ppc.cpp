...
void VM_Version::initialize() {
  ...
  jio_snprintf(buf, sizeof(buf),
               "ppc%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s",
               PPC64_ONLY("64") NOT_PPC64("32"),
               (has_fsqrt()   ? " fsqrt"   : ""),
               (has_isel()    ? " isel"    : ""),
               (has_lxarxeh() ? " lxarxeh" : ""),
               (has_cmpb()    ? " cmpb"    : ""),
...
void VM_Version::determine_features() {
  ...
  const int code_size = (num_features+1+USE_SPE_ONLY(+2)+2*7)*BytesPerInstWord; // TODO(asmundak): calculation is incorrect.
#else
  ...
  // Emit code.
  void (*test)(address addr, uintptr_t offset)=(void(*)(address addr, uintptr_t offset))(void *)a->function_entry();
  uint32_t *code = (uint32_t *)a->pc();
  // Don't use R0 in larx.
  ...
  a->lwsync();                                 // code[10] -> lwsync
#endif
  a->lqarx_unchecked(R7, R3_ARG1, R4_ARG2, 1); // code[11] -> lqarx_m
  a->vcipher(VR0, VR1, VR2);                   // code[12] -> vcipher
  a->vpmsumb(VR0, VR1, VR2);                   // code[13] -> vpmsumb
...
#ifdef USE_SPE
  unsigned int (*spe_add)(float, float) = (unsigned int(*)(float, float))(void *)a->function_entry();
  a->efsadd(R3, R3, R4);
  a->blr();
#endif
  ...
  (*test)((address)align_pointer_up(mid_of_test_area, 8), (uintptr_t)0);
...
#ifndef __NO_LWSYNC__
  if (code[feature_cntr++]) features |= lwsync_m;
#endif

#ifdef USE_SPE
  if (0x80000000 != (*spe_add)(-0.0, -0.0)) {
    features |= incorrect_single_fp_m;
  }
#endif
