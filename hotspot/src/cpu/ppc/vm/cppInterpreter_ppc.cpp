...
address CppInterpreterGenerator::generate_stack_to_stack_converter(BasicType type) {
  ...
  case T_DOUBLE:
    ...
    __ l(tmp1, BytesPerWord, from);
    __ l(tmp2, 0, from);
    __ st(tmp1, 0, tos);
    __ st(tmp2, -BytesPerWord, tos);
    // New expression stack top.
...
void CppInterpreterGenerator::generate_compute_interpreter_state(Label& stack_overflow_return) {
  ...
  __ l(max_stack, in_bytes(Method::const_offset()), R19_method);   // Max_stack holds constMethod for a while.
  ...
  __ l(mem_stack_limit, thread_(stack_overflow_limit));
  ...
  __ sli(R18_locals, parameter_count, Interpreter::logStackElementSize);
  ...
  __ sli(R0, local_count, Interpreter::logStackElementSize);
  ...
  const Register local_addr    = R28_tmp8;
  {
    ...
    // if (!is_native) {
      ...
      __ l(bytecode_addr, method_(const));
      __ addi(bytecode_addr, bytecode_addr, in_bytes(ConstMethod::codes_offset()));
    // }
    __ bind(L);
  }

  __ l(constants, in_bytes(Method::const_offset()), R19_method);
  __ l(constants, in_bytes(ConstMethod::constants_offset()), constants);

  // state->_prev_link = prev_state;
  __ st(R15_prev_state, state_(_prev_link));

  // For assertions only.
  // TODO: not needed anyway because it coincides with `_monitor_base'. remove!
  // state->_self_link = state;
  DEBUG_ONLY(__ st(R14_state, state_(_self_link));)

  // state->_thread = thread;
  __ st(R16_thread, state_(_thread));

  // state->_method = method;
  __ st(R19_method, state_(_method));

  // state->_locals = locals;
  __ st(R18_locals, state_(_locals));

  // state->_oop_temp = NULL;
  __ li(R0, 0);
  __ st(R0, state_(_oop_temp));

  // state->_last_Java_fp = *R1_SP // Use *R1_SP as fp
  __ l(R0, _abi(callers_sp), R1_SP);
  __ st(R0, state_(_last_Java_fp));
...

  {
    ...
    __ st(R0, state_(_bcp));
    __ b(label2);
    __ bind(label1);
    __ st(bytecode_addr, state_(_bcp));
    __ bind(label2);
  }


  // state->_result._to_call._callee = NULL;
  __ st(R0, state_(_result._to_call._callee));

  // state->_monitor_base = state;
  __ st(R14_state, state_(_monitor_base));
  ...
  __ st(R0, state_(_stack_limit));


  // cache = method->constants()->cache();
   __ l(R0, ConstantPool::cache_offset_in_bytes(), constants);
  // state->_constants = method->constants()->cache();
  __ st(R0, state_(_constants));
  ...
  const int mirror_offset = in_bytes(Klass::java_mirror_offset());
  {
    ...
    __ l(R11_scratch1/*pool_holder*/, ConstantPool::pool_holder_offset_in_bytes(), constants);
    __ l(R0/*lockee*/, mirror_offset, R11_scratch1/*pool_holder*/);
    __ b(label2);

    __ bind(label1);
    // else lockee = *(oop*)locals;
    __ l(R0/*lockee*/, 0, R18_locals);
    __ bind(label2);

    // monitor->set_obj(lockee);
    __ st(R0/*lockee*/, BasicObjectLock::obj_offset_in_bytes(), stack_base);
  }
  ...
  if (true /* zerolocals */ || ClearInterpreterLocals) {
    ...
    __ sli(R0, parameter_count, Interpreter::logStackElementSize);
...
void CppInterpreterGenerator::generate_counter_overflow(Label& continue_entry) {
  ...
  __ l(R19_method, state_(_method));
...
address CppInterpreterGenerator::generate_native_entry(void) {
  ...
  __ st(R21_sender_SP, _top_ijava_frame_abi(initial_caller_sp), R1_SP);

  // Save LR to caller's frame. We don't use _abi(lr) here, because it is not safe.
  __ mflr(return_pc);
  __ st(return_pc, _top_ijava_frame_abi(frame_manager_lr), R1_SP);
  ...
  // by compiled code, then R15_prev_state is invalid.
  {
    ...
    __ l(R12_scratch2/*frame_manager_returnpc*/, 0, R11_scratch1/*frame_manager_returnpc_addr*/);
...

  {
    ...
    __ l(R11_scratch1/*constants*/, in_bytes(ConstMethod::constants_offset()), R11_scratch1);
    // pool_holder = method->constants()->pool_holder();
    ...
    __ st(R0/*mirror*/, state_(_oop_temp));
...
#else
  __ stw(R3_RET, state_(w._native_lresult1));
  __ stw(R4_RET2, state_(w._native_lresult2));
#endif
  ...
  __ l(R19_method, state_(_method)); // reload method, may have moved
...
#else
  __ ld(R3_RET, state_(w._native_lresult1));
  __ ld(R4_RET2, state_(w._native_lresult2));
#endif
  ...
  __ l(R0, _top_ijava_frame_abi(frame_manager_lr), R1_SP);
  __ mtlr(R0);
  // Resize frame to get rid of a potential extension.
  __ resize_frame_to_initial_caller(R11_scratch1, R12_scratch2);
...
void CppInterpreterGenerator::lock_method(void) {
  ...
  // Pass object address.
  __ l(object, BasicObjectLock::obj_offset_in_bytes(), monitor);
...
void CppInterpreterGenerator::generate_more_monitors() {
  ...
  __ l(stack_base, state_(_stack_base));                // Old stack_base
  __ l(old_tos, state_(_stack));                        // Old tos
  __ l(stack_limit, state_(_stack_limit));              // Old stack_limit
  ...
  __ l(R12_scratch2, _top_ijava_frame_abi(frame_manager_lr), R1_SP);
  __ resize_frame(-monitor_size, R11_scratch1);// Allocate space for new monitor
  __ st(R12_scratch2, _top_ijava_frame_abi(frame_manager_lr), R1_SP);
    // Initial_caller_sp is used as unextended_sp for non initial callers.
  __ st(R1_SP, _top_ijava_frame_abi(initial_caller_sp), R1_SP);
  ...
  __ st(R1_SP, state_(_last_Java_sp));                  // Update frame_bottom

  __ st(stack_base, state_(_stack_base));               // Update stack_base
  __ st(new_tos, state_(_stack));                       // Update tos
  __ st(stack_limit, state_(_stack_limit));             // Update stack_limit
  ...
  __ sri_(n_slots, n_slots, LogBytesPerWord);           // compute number of slots to copy
  ...
  // loop
  __ bind(copy_slot);
  __ lu(slot, BytesPerWord, old_tos);                   // slot = *++old_tos;
  __ stu(slot, BytesPerWord, new_tos);                  // *++new_tos = slot;
  __ bdnz(copy_slot);
  ...
  // Restart interpreter
  __ li(R0, 0);
  __ st(R0, BasicObjectLock::obj_offset_in_bytes(), stack_base);  // Mark lock as unused
}
...
address CppInterpreterGenerator::generate_normal_entry(void) {
  ...
  if (Arguments::has_profile()) {
    ...
    __ st(R14_state, thread_(last_interpreter_fp));
  }
  ...
  if (Arguments::has_profile()) {
    ...
    __ st(R11_scratch1, thread_(last_interpreter_fp));
  }
  ...
  __ st(R3_RET, thread_(pending_exception));
  ...
  __ l(R19_method, state_(_result._to_call._callee));
  ...
  __ l(parameter_count, in_bytes(Method::const_offset()), R19_method);
  __ lhz(parameter_count, in_bytes(ConstMethod::size_of_parameters_offset()), parameter_count);
  __ sli(parameter_count, parameter_count, Interpreter::logStackElementSize);
  ...
  __ l(R15_prev_state, state_(_prev_link));
  ...
  __ sli(result_index, result_index, LogBytesPerWord);
  __ add(stub_addr, stub_addr, result_index);

  // Stub descriptor address.
  __ l(stub_addr, 0, stub_addr);
  ...
  //   msg
  {
    ...
    __ l(R3_RET, prev_state_(_stack));
  ...
  __ l(R21_tmp1, _parent_ijava_frame_abi(lr), R1_SP);
  ...
  __ pop_frame();
  __ l(R3_ARG1, _top_ijava_frame_abi(lr), R1_SP);
  ...
  __ l(R3_ARG1/*R3_RET*/, thread_(pending_exception));
  __ li(R4_ARG2, 0);
  __ st(R4_ARG2, thread_(pending_exception));
  ...
  __ l(stub_entry, state_(_result._to_call._callee_entry_point));
  ...
  //  __ l(R17, state_(_locals));

  // Load osr entry.
  __ l(R12_scratch2, state_(_result._osr._osr_entry));

  // Load address of temporary osr buffer to arg1.
  __ l(R3_ARG1, state_(_result._osr._osr_buf));
  __ mtctr(R12_scratch2);

  // Load method, gc may move it during execution of osr'd method.
  __ l(R22_tmp2, state_(_method));
...

  {
    ...
    __ st(R22_tmp2, state_(_result._to_call._callee));
...
#else
  to_fill->w._native_lresult1 = CONST64(0xdeafcafe);
  to_fill->w._native_lresult2 = CONST64(0xdeadc0de);
#endif // PPC64
#endif // ASSERT
}
