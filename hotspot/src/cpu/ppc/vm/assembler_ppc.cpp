...
#endif
  if (is_power_of_2_long((jlong) ui16)) {
    // pow2
...
#else
    clrrwi(a, s, log2_long((jlong)-ui16));
#endif
  } else {
    assert(is_uimm((jlong)ui16, 16), "must be 16-bit unsigned immediate");
    andi_(a, s, ui16);
  }
}
...
// Load a constant into register. Patchable.
...
// Load a constant, optimized, not identifyable.
...
int Assembler::load_const_optimized(Register d, long x, Register tmp, bool return_simm16_rest) {
  ...
  return retval;
#else
  const short xb = x & 0xFFFF; // Low 16-bit chunks of const.
  // Remaining part of const with compensation of sign extend
  const short xa = (x >> 16) + ((unsigned short)xb >> 15);

  if (xa == 0) { // opt 1: simm16
    li(d, xb);
    return 0;
  }

  if (d == R0) {
    const short xa0 = (x >> 16); // High 16-bit chunks of const.

    // opt 2: simm32
    lis(d, xa0);
    // ori used, could OR (R0, R0, X), in contrast with addi
    if (xb) { ori(d, d, (unsigned short)xb); }
    return 0;
  }
