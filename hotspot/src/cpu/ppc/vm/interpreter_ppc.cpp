...
address AbstractInterpreterGenerator::generate_slow_signature_handler() {
  ...
  const int SizeOfThreeInstructionsDivByTwo  = 6;

  // FIXME: use Argument:: GL: Argument names different numbers!
#ifdef PPC64
  const int max_fp_register_arguments  = 13;
#else
  const int max_fp_register_arguments  = 8;
#endif
  const int max_int_register_arguments = 6;  //starting from r5
...
#ifdef PPC64
    __ st(R4_ARG2, _abi(carg_2), target_sp);
#endif
    __ bind(L);
  }
...
#else
  // taget_sp points to back chain and LR
  // arg_c starts from offset 8
  __ addi(arg_c, target_sp, 2*BytesPerWord);
  // argcnt=0 corresponds to 3rd C argument.
  // no args parsed so far.
  __ li(argcnt, 0);
#endif
...
#ifndef PPC64
  Label move_boxedSlot_to_ARG, move_longSlot_to_ARG, move_doubleSlot_to_FARG;
#endif
  ...
  __ addi(arg_java, arg_java, -BytesPerWord);
  __ cmplwi(CCR0, argcnt, max_int_register_arguments);
  __ blt(CCR0, move_boxedSlot_to_ARG);
  __ st(intSlot, 0, arg_c);
  __ addi(arg_c, arg_c, BytesPerWord);
#endif
  __ b(loop_start);
  ...
  __ cmplwi(CCR0, argcnt, max_int_register_arguments);
  __ blt(CCR0, move_intSlot_to_ARG);
  __ lwa(intSlot, BytesPerWord, arg_java);
  __ st(intSlot, 0, arg_c);
  __ addi(arg_c, arg_c, BytesPerWord);
#endif
  __ b(loop_start);
  ...
  __ bind(do_long);
#ifdef PPC64
  __ l(intSlot, -BytesPerWord, arg_java);
  __ st(intSlot, 0, arg_c);
...
#else
  __ addi(arg_java, arg_java, - 2 * BytesPerWord);
  __ cmplwi(CCR0, argcnt, max_int_register_arguments-1);
  __ blt(CCR0, move_longSlot_to_ARG);
  __ load_const(argcnt, max_int_register_arguments); // prevent next args to registers
  __ l(intSlot, BytesPerWord, arg_java);
  __ round_to(arg_c, 2*BytesPerWord);  // align
  __ st(intSlot, 0, arg_c);
  __ l(intSlot, 2*BytesPerWord, arg_java);
  __ st(intSlot, BytesPerWord, arg_c);
  __ addi(arg_c, arg_c, 2 * BytesPerWord);
#endif
  __ b(loop_start);
...
#error "unknown OS"
  ...
  __ cmplwi(CCR0, fpcnt, max_fp_register_arguments);
  __ blt(CCR0, move_floatSlot_to_FARG);
  __ lfs(floatSlot, BytesPerWord, arg_java);
  __ stfs(floatSlot, 0, arg_c);
  __ addi(arg_c, arg_c, BytesPerWord);
#endif
  __ b(loop_start);
  ...
  __ addi(arg_java, arg_java, - 2 * BytesPerWord);
  __ cmplwi(CCR0, fpcnt, max_fp_register_arguments);
  __ blt(CCR0, move_doubleSlot_to_FARG);
  __ lfd(floatSlot, BytesPerWord, arg_java);
  __ round_to(arg_c, 2*BytesPerWord);  // align
  __ stfd(floatSlot, 0, arg_c);
  __ addi(arg_c, arg_c, 2 * BytesPerWord);
...
#ifndef PPC64
  Label move_boxed_arg, move_long_arg, move_double_arg;
  __ bind(move_boxed_arg);
  ...
  __ bind(move_long_arg); // each case must consist of 3 instructions (otherwise adapt SizeOfThreeInstructionsDivByTwo)
  __ l(R5_ARG3, BytesPerWord, arg_java);  __ l(R6_ARG4,  2*BytesPerWord, arg_java); __ b(loop_start);
  __ l(R7_ARG5, BytesPerWord, arg_java);  __ l(R8_ARG6,  2*BytesPerWord, arg_java); __ b(loop_start);
  __ l(R9_ARG7, BytesPerWord, arg_java);  __ l(R10_ARG8, 2*BytesPerWord, arg_java); __ b(loop_start);
#endif
  ...
  __ mr(R10_ARG8, intSlot); __ b(loop_start);
#else
  __ lwa(R5_ARG3, BytesPerWord, arg_java);  __ b(loop_start);
  __ lwa(R6_ARG4, BytesPerWord, arg_java);  __ b(loop_start);
  __ lwa(R7_ARG5, BytesPerWord, arg_java);  __ b(loop_start);
  __ lwa(R8_ARG6, BytesPerWord, arg_java);  __ b(loop_start);
  __ lwa(R9_ARG7, BytesPerWord, arg_java);  __ b(loop_start);
  __ lwa(R10_ARG8, BytesPerWord, arg_java); __ b(loop_start);
#endif
...
#else
  __ lfs(F1_ARG1, BytesPerWord, arg_java);   __ b(loop_start);
  __ lfs(F2_ARG2, BytesPerWord, arg_java);   __ b(loop_start);
  __ lfs(F3_ARG3, BytesPerWord, arg_java);   __ b(loop_start);
  __ lfs(F4_ARG4, BytesPerWord, arg_java);   __ b(loop_start);
  __ lfs(F5_ARG5, BytesPerWord, arg_java);   __ b(loop_start);
  __ lfs(F6_ARG6, BytesPerWord, arg_java);   __ b(loop_start);
  __ lfs(F7_ARG7, BytesPerWord, arg_java);   __ b(loop_start);
  __ lfs(F8_ARG8, BytesPerWord, arg_java);   __ b(loop_start);
#endif

#ifndef PPC64
  __ bind(move_double_arg); // each case must consist of 2 instructions (otherwise adapt LogSizeOfTwoInstructions)
  __ lfd(F1_ARG1, BytesPerWord, arg_java);   __ b(loop_start);
  __ lfd(F2_ARG2, BytesPerWord, arg_java);   __ b(loop_start);
  __ lfd(F3_ARG3, BytesPerWord, arg_java);   __ b(loop_start);
  __ lfd(F4_ARG4, BytesPerWord, arg_java);   __ b(loop_start);
  __ lfd(F5_ARG5, BytesPerWord, arg_java);   __ b(loop_start);
  __ lfd(F6_ARG6, BytesPerWord, arg_java);   __ b(loop_start);
  __ lfd(F7_ARG7, BytesPerWord, arg_java);   __ b(loop_start);
  __ lfd(F8_ARG8, BytesPerWord, arg_java);   __ b(loop_start);
#endif
#endif

  __ bind(move_intSlot_to_ARG);
  __ sli(R0, argcnt, LogSizeOfTwoInstructions);
#ifndef PPC64
  __ addi(argcnt, argcnt, 1);
#endif
  ...
  __ sli(R0, fpcnt, LogSizeOfTwoInstructions);
...
#ifndef PPC64
  __ bind(move_boxedSlot_to_ARG);
  __ sli(R0, argcnt, LogSizeOfTwoInstructions);
  __ addi(argcnt, argcnt, 1);
  __ load_const(R11_scratch1, move_boxed_arg); // Label must be bound here.
  __ add(R11_scratch1, R11_scratch1, R0);
  __ mtctr(R11_scratch1/*branch_target*/);
  __ bctr();

  __ bind(move_longSlot_to_ARG);
  __ addi(argcnt, argcnt, 1);
  __ andi(argcnt, argcnt, 0x0E);
  __ mulli(R0, argcnt, SizeOfThreeInstructionsDivByTwo);
  __ load_const(R11_scratch1, move_long_arg); // Label must be bound here.
  __ add(R11_scratch1, R11_scratch1, R0);
  __ addi(argcnt, argcnt, 2);
  __ mtctr(R11_scratch1/*branch_target*/);
  __ bctr();

#ifndef USE_SPE
  __ bind(move_doubleSlot_to_FARG);
  __ sli(R0, fpcnt, LogSizeOfTwoInstructions);
  __ addi(fpcnt, fpcnt, 1);
  __ load_const(R11_scratch1, move_double_arg); // Label must be bound here.
  __ add(R11_scratch1, R0, R11_scratch1);
  __ mtctr(R11_scratch1/*branch_target*/);
  __ bctr();
#endif
...
address AbstractInterpreterGenerator::generate_result_handler_for(BasicType type) {
  ...
  case T_INT:
     // sign extend 32 bits
     PPC64_ONLY(__ extsw(R3_RET, R3_RET));
     break;
  case T_LONG:
     break;
  case T_OBJECT:
...
address InterpreterGenerator::generate_accessor_entry(void) {
  ...
  __ l(Rclass_or_obj, Interpreter::stackElementSize, CC_INTERP_ONLY(R17_tos) NOT_CC_INTERP(R15_esp));
  __ lwz(Rjvmti_mode, thread_(interp_only_mode));
  __ cmpi(CCR1, Rclass_or_obj, 0);
  ...
  __ l(Rconst_method, in_bytes(Method::const_offset()), R19_method);
  __ l(Rcpool_cache, in_bytes(ConstMethod::constants_offset()), Rconst_method);

  __ lhz(Rcodes, in_bytes(ConstMethod::codes_offset()) + 2, Rconst_method); // Lower half of 32 bit field.
  __ l(Rcpool_cache, ConstantPool::cache_offset_in_bytes(), Rcpool_cache);
  ...
  __ l(Rflags, in_bytes(cp_base_offset) + in_bytes(ConstantPoolCacheEntry::flags_offset()), Rcpool_cache);
  __ l(Roffset, in_bytes(cp_base_offset) + in_bytes(ConstantPoolCacheEntry::f2_offset()), Rcpool_cache);
  ...
  if (branch_table[ltos] == 0) { // generate only once
    ...
    __ lx(R3_RET, Rclass_or_obj, Roffset);
    __ beq(CCR6, Lacquire);
    __ blr();
#else // PPC64
    __ align(32, 28, 28); // align load
    {
      ...
      __ l(R3_RET, - Interpreter::stackElementSize, R15_esp);
      __ l(R4_RET2, 0, R15_esp);
#else // SPE_ATOMIC
      __ evlddx_aligned(R3_RET, Rclass_or_obj, Roffset);
      __ evmergehi(R4_RET2, R3_RET, R3_RET);
#endif // SPE_ATOMIC
      __ b(Lacquire);
      __ b(LisVolatile); // volatile entry point (one instruction before non-volatile_entry point)
    }
    branch_table[ltos] = __ pc(); // non-volatile_entry point
    __ lx(R3_RET, Rclass_or_obj, Roffset);
    __ addi(R11_scratch1, R11_scratch1, wordSize);
    __ lx(R4_RET2, Rclass_or_obj, R11_scratch1);
    __ blr();
#endif // PPC64
  }
...
address InterpreterGenerator::generate_Reference_get_entry(void) {
  ...
  if (UseG1GC) {
    ...
    __ l(R3_RET, Interpreter::stackElementSize, CC_INTERP_ONLY(R17_tos) NOT_CC_INTERP(R15_esp)); // get receiver
