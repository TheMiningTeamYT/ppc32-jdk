...
// We use an illtrap for marking a method as not_entrant or zombie iff !TrapBasedNotEntrantChecks
// Work around a C++ compiler bug which changes 'this'
bool NativeInstruction::is_sigill_zombie_not_entrant_at(address addr) {
  assert(!TrapBasedNotEntrantChecks, "precondition");
    ...
    if (MacroAssembler::get_address_of_calculate_address_from_global_toc_at(addr, cb->content_begin()) !=
      ...
      const int other_insn_offset =
        MacroAssembler::patch_calculate_address_from_global_toc_at(addr, cb->content_begin(),
                                                                   (address)data);
      const address start = other_insn_offset < 0 ? addr + other_insn_offset : addr;
      const int range = BytesPerInstWord + abs(other_insn_offset);
...
address NativeCallTrampolineStub::encoded_destination_addr() const {
  ...
  if (!MacroAssembler::is_ld_largeoffset(instruction_addr)) {
    ...
    assert(MacroAssembler::is_ld_largeoffset(instruction_addr),
           "must be a l with large offset (from the constant pool)");
  }
