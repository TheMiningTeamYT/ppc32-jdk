...
#else // __NO_LWSYNC__
#define inlasm_lwsync()   __asm__ __volatile__ ("  sync" : : : "memory");
#endif // __NO_LWSYNC__
...
inline intptr_t OrderAccess::load_ptr_acquire(volatile intptr_t*   p) { return (intptr_t)load_acquire((volatile ptr_type*)p); }
inline void*    OrderAccess::load_ptr_acquire(volatile void*       p) { return (void*)   load_acquire((volatile ptr_type*)p); }
inline void*    OrderAccess::load_ptr_acquire(const volatile void* p) { return (void*)   load_acquire((volatile ptr_type*)p); }
