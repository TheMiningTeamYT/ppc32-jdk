...
extern "C" void pnsc() {
  ...
  assert(t->is_Java_thread(), "should not call this");
...
void report_java_out_of_memory(const char* message) {
  ...
  static Thread* reporting_thread;
  static jint out_of_memory_reported = 0;

  const bool sync_oom = PrintJVMStateOnOutOfMemoryError;
  ...
  if (!sync_oom) {
    ...
    // serialize threads on OOM, finish flags handling before
    // any thread post jvmti::ResourceExhausted
    MutexLockerEx m(OutOfMemoryError_lock);
    switch (report_state) {
    case not_reported:
      report_state = reporting_now;
      reporting_thread = Thread::current();
      break;
    case reporting_now:
      if (reporting_thread != Thread::current()) {
        do {
          OutOfMemoryError_lock->wait(false, 0);
        } while (report_state != done_reporting);
      } else {
        tty->print_cr("Recursive lock of OutOfMemoryError");
        pnsc();
      }
      // fallthrough
    case done_reporting:
  ...
  if (PrintJVMStateOnOutOfMemoryError) {
    VM_PrintThreads op;
    VMThread::execute(&op);
    TraceTime t("Class histogram");
    VM_GC_HeapInspection op1(gclog_or_tty, false/*force full GC before*/);
  ...
  if (sync_oom) {
    MutexLockerEx m(OutOfMemoryError_lock);
    assert(report_state == reporting_now, "");
    report_state = done_reporting;
    OutOfMemoryError_lock->notify_all();
  }
}
