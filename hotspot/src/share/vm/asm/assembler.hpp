...
class AbstractAssembler : public ResourceObj  {
 ...
 public:
  ...
  static int64_t min_simm(int nbits) { return -(int64_t(1) << (nbits - 1))    ; }
  static int64_t max_simm(int nbits) { return  (int64_t(1) << (nbits - 1)) - 1; }
  ...
  static bool is_simm(int64_t x, int nbits) { return min_simm(nbits) <= x && x <= max_simm(nbits); }
  ...
  static bool is_simm26(int64_t x) { return is_simm(x, 26); }
