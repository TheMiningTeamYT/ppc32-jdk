...
#else
#if defined(SPARC) || (!defined(VM_LITTLE_ENDIAN) && defined(MIPS32))
      // For SPARC (and any other BE arch, actually) we have to swap high and low words.
      jlong res = jlong_from((jint)low->get_int(), (jint)value->get_int());
#else
      jlong res = jlong_from((jint)value->get_int(), (jint)low->get_int());
#endif //SPARC || (!VM_LITTLE_ENDIAN && MIPS32)
#endif
