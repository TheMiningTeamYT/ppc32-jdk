...
#if defined(AARCH32) && (!defined(__ARM_ARCH) || __ARM_ARCH > 5)
    // on aarch32 check of the flag became really hot, so inlining the value
    // (and therefore always use fences) gives considerable performance boost.
    return true;
#else
