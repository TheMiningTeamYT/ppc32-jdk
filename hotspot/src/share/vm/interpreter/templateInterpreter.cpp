...
#define method_entry(kind)                                                                    \
  ...
  if (UseAdler32Intrinsics) {
    method_entry(java_util_zip_Adler32_updateBytes)
    method_entry(java_util_zip_Adler32_updateByteBuffer)
  }

  if (UseAESIntrinsics) {
    method_entry(com_sun_crypto_provider_AESCrypt_encryptBlock)
    method_entry(com_sun_crypto_provider_AESCrypt_decryptBlock)
    if (StubRoutines::cipherBlockChaining_encryptAESCrypt()) {
      method_entry(com_sun_crypto_provider_CipherBlockChaining_encrypt)
    }
    if (StubRoutines::cipherBlockChaining_decryptAESCrypt()) {
  ...
  if (UseGHASHIntrinsics) {
      method_entry(com_sun_crypto_provider_ghash_processBlocks)
  }
#endif
...
class DynInterpreterCodeletClosure : public TemplateInterpreter::InterpreterCodeletClosure {
  GrowableArray<JvmtiCodeBlobDesc*>* _descs;
public:
  DynInterpreterCodeletClosure(GrowableArray<JvmtiCodeBlobDesc*>* descs)
    : _descs(descs)
  { }

  void do_cl(address start, address end, const char* name) {
    JvmtiCodeBlobDesc* scb = new JvmtiCodeBlobDesc(name, start, end);
    _descs->append(scb);
  }
};

bool TemplateInterpreter::try_add_bc_for_jvmti(CodeBlob* cb, GrowableArray<JvmtiCodeBlobDesc*>* blob_descs) {
  if (cb->code_begin() == _code->code_start()) {
    DynInterpreterCodeletClosure cl(blob_descs);
    interpreter_codelet_do(&cl);
    return true;
  }
  return false;
}

void TemplateInterpreter::interpreter_codelet_do(TemplateInterpreter::InterpreterCodeletClosure* cl) {
  switch (JVMTIInterpreterNameMode) {
  case 0:
    cl->do_cl(_code->code_start(), _code->code_end(), "Interpreter");
    break;
  default:
    for (Stub* s = _code->first(); s != NULL; s = _code->next(s)) {
      InterpreterCodelet* is = (InterpreterCodelet*)s;
      char buf[128];
      snprintf(buf, sizeof(buf), "Interpreter: %s", is->bytecode() != Bytecodes::_illegal ?
          Bytecodes::name(is->bytecode()) :
          is->description());
      cl->do_cl(is->code_begin(), is->code_end(), buf);
    }
    break;
  }
}
