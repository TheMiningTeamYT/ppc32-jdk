...
class InitInterpreterCodeletClosure : public TemplateInterpreter::InterpreterCodeletClosure {
public:
  void do_cl(address start, address end, const char* name) {
    JvmtiExport::post_dynamic_code_generated(name, start, end);
  }
};
...
void interpreter_init() {
  ...
  if (JvmtiExport::should_post_dynamic_code_generated()) {
    InitInterpreterCodeletClosure cl;
    TemplateInterpreter::interpreter_codelet_do(&cl);
  }
}
...
AbstractInterpreter::MethodKind AbstractInterpreter::method_kind(methodHandle m) {
  ...
  if (UseAdler32Intrinsics) {
    // Use optimized stub code for Adler32 native methods.
    switch (m->intrinsic_id()) {
      case vmIntrinsics::_updateBytesAdler32 : return java_util_zip_Adler32_updateBytes;
      case vmIntrinsics::_updateByteBufferAdler32 : return java_util_zip_Adler32_updateByteBuffer;
    }
  }
#endif
  ...
  if (UseGHASHIntrinsics) {
    // Use optimized stub code for GHASH processBlocks method.
    switch (m->intrinsic_id()) {
      case vmIntrinsics::_ghash_processBlocks  : return com_sun_crypto_provider_ghash_processBlocks;
    }
  }
...
void AbstractInterpreter::print_method_kind(MethodKind kind) {
  switch (kind) {
    ...
    case com_sun_crypto_provider_ghash_processBlocks : tty->print("com_sun_crypto_provider_ghash_processBlocks"); break;
#endif
