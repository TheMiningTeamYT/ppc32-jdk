...
#ifdef MIPS32_FPU_LE
      if ((t->base() != Type::Long) && OptoReg::is_reg(regnum) ) {
        // double load instruction stores hi word in lower register and vice versa.
        // so that dopt stored value is word-swapped
        // Word-swap is not required in case of doubles on stack
        array->append(new_loc_value( _regalloc,              regnum   , Location::normal ));
        array->append(new_loc_value( _regalloc, OptoReg::add(regnum,1), Location::normal ));
      } else
#endif
...
void Compile::Process_OopMap_Node(MachNode *mach, int current_offset) {
  ...
  if( !mach->is_MachCall() ) {
    mcall = NULL;
    // MIPS64 had adjusting of safepoint_pc_offset since Safepoint
    // node calculated poll_page address by itself. C2 can provide
    // poll_page address, by implementing
    //   - SafePointNode::needs_polling_address_input { true }
    //   - instruct { match(Safepoint poll) }
    // So we don't need to adjust offset here.
