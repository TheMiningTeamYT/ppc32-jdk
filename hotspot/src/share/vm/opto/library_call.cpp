...
CallGenerator* Compile::make_vm_intrinsic(ciMethod* m, bool is_virtual) {
  ...
  case vmIntrinsics::_cipherBlockChaining_encryptAESCrypt:
    if (!UseAESIntrinsics) return NULL;
    if (!StubRoutines::cipherBlockChaining_encryptAESCrypt()) return NULL;
  ...
  case vmIntrinsics::_cipherBlockChaining_decryptAESCrypt:
    if (!UseAESIntrinsics) return NULL;
    if (!StubRoutines::cipherBlockChaining_decryptAESCrypt()) return NULL;
    // these two require the predicated logic
    predicates = 1;
    break;
  ...
  case vmIntrinsics::_digestBase_implCompressMB:
    ...
    if (!(StubRoutines::sha1_implCompressMB()   ||
          StubRoutines::sha256_implCompressMB() ||
          StubRoutines::sha512_implCompressMB())) return NULL;
    predicates = 3;
    break;
...
#define FN_PTR(f) CAST_FROM_FN_PTR(address, f)
  ...
  case vmIntrinsics::_dcos:   return Matcher::match_rule_supported(Op_CosD)   ? inline_trig(id) :
    runtime_math(OptoRuntime::Math_D_D_Type(), FN_PTR(SharedRuntime::dcos),   "COS");
  case vmIntrinsics::_dsin:   return Matcher::match_rule_supported(Op_SinD)   ? inline_trig(id) :
    runtime_math(OptoRuntime::Math_D_D_Type(), FN_PTR(SharedRuntime::dsin),   "SIN");
  case vmIntrinsics::_dtan:   return Matcher::match_rule_supported(Op_TanD)   ? inline_trig(id) :
    runtime_math(OptoRuntime::Math_D_D_Type(), FN_PTR(SharedRuntime::dtan),   "TAN");

  case vmIntrinsics::_dlog:   return Matcher::match_rule_supported(Op_LogD)   ? inline_math(id) :
    runtime_math(OptoRuntime::Math_D_D_Type(), FN_PTR(SharedRuntime::dlog),   "LOG");
  case vmIntrinsics::_dlog10: return Matcher::match_rule_supported(Op_Log10D) ? inline_math(id) :
    runtime_math(OptoRuntime::Math_D_D_Type(), FN_PTR(SharedRuntime::dlog10), "LOG10");

  case vmIntrinsics::_dsqrt:  return Matcher::match_rule_supported(Op_SqrtD) ? inline_math(id) :
#ifdef MIPS
    runtime_math(OptoRuntime::Math_D_D_Type(), FN_PTR(SharedRuntime::dsqrt),  "DSQRT")
#else
      false
#endif
      ;
  case vmIntrinsics::_dabs:   return Matcher::match_rule_supported(Op_AbsD)   ? inline_math(id) :
#ifdef MIPS
    runtime_math(OptoRuntime::Math_D_D_Type(), FN_PTR(SharedRuntime::dabs),   "DABS")
#else
      false
#endif
      ;

  case vmIntrinsics::_dexp:   return Matcher::match_rule_supported(Op_ExpD)   ? inline_exp()    :
    runtime_math(OptoRuntime::Math_D_D_Type(),  FN_PTR(SharedRuntime::dexp),  "EXP");
  case vmIntrinsics::_dpow:   return Matcher::match_rule_supported(Op_PowD)   ? inline_pow()    :
...
bool LibraryCallKit::inline_fp_conversions(vmIntrinsics::ID id) {
  ...
  case vmIntrinsics::_doubleToLongBits: {
    if (!Matcher::match_rule_supported(Op_CmpD))
      return false;
  ...
  case vmIntrinsics::_floatToIntBits: {
    if (!Matcher::match_rule_supported(Op_CmpF))
      return false;
    // two paths (plus control) merge in a wood
    RegionNode *r = new (C) RegionNode(3);
...
LibraryCallKit::generate_generic_arraycopy(const TypePtr* adr_type,
  ...
  if (!CCallingConventionRequiresIntsAsLongs) {
    ...
    dest_offset = ConvI2L(dest_offset);
    copy_length = ConvI2L(copy_length);
    call = make_runtime_call(RC_LEAF|RC_NO_FP,
                      OptoRuntime::generic_arraycopy_Type(),
                      copyfunc_addr, "generic_arraycopy", adr_type,
                      src, src_offset, top(), dest, dest_offset, top(),
                      copy_length, top());
  }
...
bool LibraryCallKit::inline_aescrypt_Block(vmIntrinsics::ID id) {
  ...
  const char *stubName;
  assert(UseAESIntrinsics, "need AES instruction support");
...
bool LibraryCallKit::inline_cipherBlockChaining_AESCrypt(vmIntrinsics::ID id) {
  ...
  switch(id) {
  case vmIntrinsics::_cipherBlockChaining_encryptAESCrypt:
    stubAddr = StubRoutines::cipherBlockChaining_encryptAESCrypt_inline();
    stubName = "cipherBlockChaining_encryptAESCrypt";
    break;
  case vmIntrinsics::_cipherBlockChaining_decryptAESCrypt:
    stubAddr = StubRoutines::cipherBlockChaining_decryptAESCrypt_inline();
