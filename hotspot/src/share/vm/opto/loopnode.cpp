...
void PhaseIdealLoop::build_and_optimize(bool do_split_ifs, bool skip_loop_opts) {
  ...
  if (_verify_me) {             // Nested verify pass?
    // Have to perform verification here to avoid verify loop memory being freed
    VectorSet visited(Thread::current()->resource_area());

    fail = 0;
    _verify_me->verify_compare( C->root(), this, visited );
    assert( fail == 0, "verify loops failed" );
    // Verify loop structure is the same
    _verify_me->_ltree_root->verify_tree(_ltree_root, NULL);
