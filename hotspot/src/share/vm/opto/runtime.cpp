...
#ifdef __SOFTFP__
const TypeFunc* OptoRuntime::f2f_Type() {
  const Type **fields = TypeTuple::fields(1);
  fields[TypeFunc::Parms+0] = Type::FLOAT;
  const TypeTuple *domain = TypeTuple::make(TypeFunc::Parms+1, fields);
...
const TypeFunc* OptoRuntime::ff2f_Type() {
  const Type **fields = TypeTuple::fields(2);
  fields[TypeFunc::Parms+0] = Type::FLOAT;
  fields[TypeFunc::Parms+1] = Type::FLOAT;
  const TypeTuple *domain = TypeTuple::make(TypeFunc::Parms+2, fields);
...
const TypeFunc* OptoRuntime::fcmp_Type() {
  const Type **fields = TypeTuple::fields(2);
  fields[TypeFunc::Parms+0] = Type::FLOAT;
  fields[TypeFunc::Parms+1] = Type::FLOAT;
  const TypeTuple *domain = TypeTuple::make(TypeFunc::Parms+2, fields);
...
const TypeFunc* OptoRuntime::dcmp_Type() {
...
const TypeFunc* OptoRuntime::f2i_Type() {
  const Type **fields = TypeTuple::fields(1);
  fields[TypeFunc::Parms+0] = Type::FLOAT;
  const TypeTuple *domain = TypeTuple::make(TypeFunc::Parms+1, fields);
...
const TypeFunc* OptoRuntime::d2i_Type() {
  const Type **fields = TypeTuple::fields(2);
  fields[TypeFunc::Parms+0] = Type::DOUBLE;
  fields[TypeFunc::Parms+1] = Type::HALF;
  const TypeTuple *domain = TypeTuple::make(TypeFunc::Parms+2, fields);
...
const TypeFunc* OptoRuntime::i2f_Type() {
  const Type **fields = TypeTuple::fields(1);
  fields[TypeFunc::Parms+0] = TypeInt::INT;
  const TypeTuple *domain = TypeTuple::make(TypeFunc::Parms+1, fields);
...
const TypeFunc* OptoRuntime::i2d_Type() {
  const Type **fields = TypeTuple::fields(1);
  fields[TypeFunc::Parms+0] = TypeInt::INT;
  const TypeTuple *domain = TypeTuple::make(TypeFunc::Parms+1, fields);
...
const TypeFunc* OptoRuntime::f2d_Type() {
  const Type **fields = TypeTuple::fields(1);
  fields[TypeFunc::Parms+0] = Type::FLOAT;
  const TypeTuple *domain = TypeTuple::make(TypeFunc::Parms+1, fields);
...
const TypeFunc* OptoRuntime::d2f_Type() {
  const Type **fields = TypeTuple::fields(2);
  fields[TypeFunc::Parms+0] = Type::DOUBLE;
  fields[TypeFunc::Parms+1] = Type::HALF;
  const TypeTuple *domain = TypeTuple::make(TypeFunc::Parms+2, fields);
...
const TypeFunc* OptoRuntime::l2d_Type() {
  const Type **fields = TypeTuple::fields(2);
  fields[TypeFunc::Parms+0] = TypeLong::LONG;
  fields[TypeFunc::Parms+1] = Type::HALF;
  const TypeTuple *domain = TypeTuple::make(TypeFunc::Parms+2, fields);
...
const TypeFunc* OptoRuntime::f2l_Type() {
  const Type **fields = TypeTuple::fields(1);
  fields[TypeFunc::Parms+0] = Type::FLOAT;
  const TypeTuple *domain = TypeTuple::make(TypeFunc::Parms+1, fields);
...
const TypeFunc* OptoRuntime::d2l_Type() {
  const Type **fields = TypeTuple::fields(2);
  fields[TypeFunc::Parms+0] = Type::DOUBLE;
  fields[TypeFunc::Parms+1] = Type::HALF;
  const TypeTuple *domain = TypeTuple::make(TypeFunc::Parms+2, fields);
...
#ifdef __SOFTFP__
const TypeFunc* OptoRuntime::Math_LL_L_Type() {
  const Type **fields = TypeTuple::fields(4);
  fields[TypeFunc::Parms+0] = TypeLong::LONG;
  fields[TypeFunc::Parms+1] = Type::HALF;
...
static const TypeFunc* make_arraycopy_Type(ArrayCopyType act) {
  ...
  // Usually, arraycopy called through GraphKit::gen_stub which do ConvI2L,
  // but ac_generic called directly, so caller is responsible for making ConvI2L
  const bool cc_i2l = CCallingConventionRequiresIntsAsLongs && act == ac_generic;
  if (cc_i2l) {
    num_args += 3;
  }
  ...
  if (num_size_args == 0) {
    ...
    fields[argp++] = TypeInt::INT;      // src_pos
      ...
      fields[argp++] = Type::HALF; // must add an additional half for a long
  }
  }
  ...
  if (num_size_args == 0) {
    ...
    fields[argp++] = TypeInt::INT;      // length
      ...
      fields[argp++] = TypeLong::LONG;  // dest_pos
      fields[argp++] = Type::HALF;      // must add an additional half for a long
      fields[argp++] = TypeLong::LONG;  // length
      fields[argp++] = Type::HALF;      // must add an additional half for a long
    }
  }
