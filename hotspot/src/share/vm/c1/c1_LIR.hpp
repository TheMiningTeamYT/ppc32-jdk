...
#ifndef __SOFTFP__
      return is_pointer() ? pointer()->is_float_kind() : (kind_field() == fpu_register);
#else
      return is_pointer() ?
          pointer()->is_float_kind() :
          (kind_field() == fpu_register) ||
          (kind_field() == cpu_register && (type() == T_FLOAT || type() == T_DOUBLE));
#endif
  }
  bool is_oop() const;
...
#endif // X86
#if defined(SPARC) || defined(ARM) || defined(AARCH64) || defined(AARCH32) || (defined(PPC) && !defined(USE_SPE))
...
#if defined(X86) || defined(ARM) || defined(AARCH64) || defined(AARCH32) || defined(OPENJDK_PPC32)
...
class LIR_List: public CompilationResourceObj {
 ...
 public:
  ...
  void mul (LIR_Opr left, LIR_Opr right, LIR_Opr res, LIR_Opr tmp_opr = LIR_OprFact::illegalOpr) { append(new
  LIR_Op2(lir_mul, left, right, res, tmp_opr)); }
...
class LIR_OpVisitState: public StackObj {
 public:
  ...
  enum {
    maxNumberOfOperands = 21, // max known reg args are for PPC hf: 8 gp regs and 13 fp regs
    maxNumberOfInfos = 4
  };
