...
void LIRItem::load_item_force(LIR_Opr reg) {
  LIR_Opr r = result();
  if (r != reg) {
#if !defined(ARM) && !defined(E500V2) && !defined(AARCH32)
    if (r->type() != reg->type()) {
...
void LIRGenerator::arithmetic_op(Bytecodes::Code code, LIR_Opr result, LIR_Opr left, LIR_Opr right, bool is_strictfp, LIR_Opr tmp_op, CodeEmitInfo* info) {
  ...
  switch(code) {
    ...
    case Bytecodes::_lmul:  __ mul(left_op, right_op, result_op, tmp_op); break;
...
#if defined(AARCH32) || defined(OPENJDK_PPC32)
...
ciKlass* LIRGenerator::profile_type(ciMethodData* md, int md_base_offset, int md_offset, intptr_t profiled_k,
  ...
  if (mdp == LIR_OprFact::illegalOpr) {
    ...
    if (md_base_offset != 0) {
#ifdef AARCH32
      LIR_Opr base_offset_opr = new_register(T_INT);
      // it is impossible to encode "lea r1, r1, 1020" without tmp register
      // since displacement value is too big for any arithmetic op
      __ move(LIR_OprFact::intptrConst(md_base_offset), base_offset_opr);
      LIR_Address* base_type_address =  new LIR_Address(mdp, base_offset_opr, T_ADDRESS);
#else
...
void LIRGenerator::do_Base(Base* x) {
  ...
  if (method()->is_synchronized()) {
    ...
    if (method()->is_synchronized() && GenerateSynchronizationCode) {
      LIR_Opr lock = PPC_ONLY(syncLockOpr()) NOT_PPC(new_register(T_INT));
