...
#define FUNCTION_CASE(a, f) \
...
#if defined(__SOFTFP__) || defined(OPENJDK_PPC32)
  FUNCTION_CASE(entry, SharedRuntime::dsqrt);
#endif
  ...
  FUNCTION_CASE(entry, StubRoutines::ghash_processBlocks());
#endif
  FUNCTION_CASE(entry, StubRoutines::aescrypt_encryptBlock());
  FUNCTION_CASE(entry, StubRoutines::aescrypt_decryptBlock());
  FUNCTION_CASE(entry, StubRoutines::cipherBlockChaining_encryptAESCrypt());
  FUNCTION_CASE(entry, StubRoutines::cipherBlockChaining_decryptAESCrypt());
...
//TODO: remove when ppc patching will be fixed
...
#if defined(SPARC) || defined(PPC32) || defined(MIPS)
