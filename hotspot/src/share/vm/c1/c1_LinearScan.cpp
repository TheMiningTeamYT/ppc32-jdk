...
// made by Azul Systems, Inc.  The dates of such changes are 2013-2019.
  ...
  if (assigned_reg >= nof_regs) {
    ...
    switch (type) {
      ...
      if(hasFPU()) {
        ...
        assert(assigned_reg >= pd_first_cpu_reg && assigned_reg <= pd_last_cpu_reg, "no cpu register");
        assert(interval->assigned_regHi() == any_reg, "must not have hi register");
        return LIR_OprFact::single_softfp(assigned_reg);
#endif
#endif // __SOFTFP__
      case T_INT: {
        assert(assigned_reg >= pd_first_cpu_reg && assigned_reg <= pd_last_cpu_reg, err_msg("no cpu register %d", assigned_reg));
        assert(interval->assigned_regHi() == any_reg, "must not have hi register");
      ...
      case T_LONG: {
        ...
        assert(assigned_reg >= pd_first_cpu_reg && assigned_reg <= pd_last_cpu_reg, "no cpu register");
        assert(num_physical_regs(type) == 1 ||
        ...
        assert(num_physical_regs(type) == 1 || assigned_reg < assigned_regHi,
               "register numbers must be sorted (ensure that e.g. a move from eax,ebx to ebx,eax can not occur)");
        assert((assigned_regHi != any_reg) ^ (num_physical_regs(type) == 1), "must be match");
        if (requires_adjacent_regs(type)) {
        ...
        if (type == T_DOUBLE) {
          if (num_physical_regs(type) == 1)
            return LIR_OprFact::double_softfp(assigned_reg, assigned_reg);
          else
           return LIR_OprFact::double_softfp(assigned_reg, assigned_regHi);
        }
#endif
...
#elif defined(ARM32) || defined(AARCH32) || defined(MIPS32)
        assert(assigned_reg >= pd_first_fpu_reg && assigned_reg <= pd_last_fpu_reg, "no fpu register");
        assert(interval->assigned_regHi() >= pd_first_fpu_reg && interval->assigned_regHi() <= pd_last_fpu_reg, "no fpu register");
        assert(assigned_reg % 2 == 0 && assigned_reg + 1 == interval->assigned_regHi(), "must be sequential and even");
...
#if defined(OPENJDK_PPC32) && defined(USE_SPE)
      ShouldNotReachHere();
#else
...
#if defined(OPENJDK_PPC32) && defined(USE_SPE)
      if (opr->type() == T_DOUBLE && rname_first == rname_second) {
        rname_second = rname_first->next();
      }
#endif
...
#if !defined(MIPS64) && !defined(MIPS32)  //MIPS64 not support lir_cmp. same as openjdk6.
