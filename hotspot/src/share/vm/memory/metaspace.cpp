...
VirtualSpaceNode::VirtualSpaceNode(size_t bytes) : _top(NULL), _next(NULL), _rs(), _container_count(0) {
  ...
  if (DumpSharedSpaces) {
    ...
    // Allocating SharedSpace in a memory region lower than max_method_code_size (64K-1) is forbidden,
    // frame::is_bci cannot distinguish the difference between bcp and bci
    shared_base = MAX2(shared_base, (char*)align_ptr_up((char*)max_method_code_size, Metaspace::reserve_alignment()));
