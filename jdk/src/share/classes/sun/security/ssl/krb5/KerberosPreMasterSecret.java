...
final class KerberosPreMasterSecret {
    ...
    KerberosPreMasterSecret(ProtocolVersion currentVersion,
        ...
        SecureRandom generator, ByteBuffer input,
        EncryptionKey sessionKey) throws IOException {

         // Extract encrypted premaster secret from message
         encrypted = Record.getBytes16(input);

         if (SSLLogger.isOn && SSLLogger.isOn("handshake")) {
            if (encrypted != null) {
                SSLLogger.fine("encrypted premaster secret", encrypted);
            }
         }
        ...
        try {
            ...
            if (SSLLogger.isOn && SSLLogger.isOn("handshake")) {
                 SSLLogger.fine("Kerberos PreMasterSecret version: "
                        + protocolVersion);
            }
        } catch (Exception e) {
        ...
        boolean versionMismatch = (protocolVersion.compare(clientVersion) != 0);
         ...
         */
        if (versionMismatch && (protocolVersion.compare(ProtocolVersion.TLS10) <= 0)) {
            versionMismatch = (protocolVersion.compare(clientVersion) != 0);
        }
         ...
         */
         if ((preMaster == null) || (preMaster.length != 48)
                || versionMismatch) {
            if (SSLLogger.isOn && SSLLogger.isOn("handshake")) {
                SSLLogger.fine("Kerberos PreMasterSecret error, "
                                   + "generating random secret");
                if (preMaster != null) {
                    SSLLogger.fine("Invalid secret", preMaster);
                }
            }
