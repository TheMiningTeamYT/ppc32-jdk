...
public class SSLSocketKeyLimit {
    ...
    public static void main(String args[]) throws Throwable {
        if (args[0].compareTo("p") != 0) {
            ...
            // -XX:+UseOpenJSSE overwrites jdk.tls.keyLimits property
            // of the test security property file.
            // Remove -XX:+UseOpenJSSE from the VM options
            List<String> optList = new ArrayList<String>();
            String[] vmOptList = Utils.getTestJavaOpts();
            for (int i=0; i<vmOptList.length; i++) {
                if (!vmOptList[i].equals("-XX:+UseOpenJSSE"))
                    optList.add(vmOptList[i]);
            }
            optList.add("-Dtest.src=" + System.getProperty("test.src"));
            optList.add("-Dtest.jdk=" + System.getProperty("test.jdk"));
            optList.add("-Djavax.net.debug=ssl,handshake");
            optList.add("-Djava.security.properties=" + f.getName());

            System.out.println("TestJavaOpts: " + String.join(" ", optList));

            optList.add("SSLSocketKeyLimit");
            optList.add("p");
            optList.add(args[1]);

            ProcessBuilder pb = ProcessTools.createJavaProcessBuilder(false, optList.toArray(new String[0]));
