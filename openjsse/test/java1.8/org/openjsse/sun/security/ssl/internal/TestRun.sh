...
${COMPILEJAVA}${FILESEP}bin${FILESEP}javac ${TESTJAVACOPTS} ${TESTTOOLVMOPTS} -d . \
    ${TESTSRC}${FILESEP}org${FILESEP}openjsse${FILESEP}sun${FILESEP}security${FILESEP}ssl${FILESEP}TestHkdf.java
${COMPILEJAVA}${FILESEP}bin${FILESEP}jar ${TESTTOOLVMOPTS} -cvf hkdf.jar \
    org${FILESEP}openjsse${FILESEP}sun${FILESEP}security${FILESEP}ssl${FILESEP}TestHkdf*.class
...
${TESTJAVA}${FILESEP}bin${FILESEP}java ${TESTVMOPTS} \
    -Dtest.src=${TESTSRC} \
    -Xbootclasspath/a:"hkdf.jar"${PATHSEP}${COMPILEJAVA}${FILESEP}jre${FILESEP}lib${FILESEP}ext${FILESEP}openjsse.jar \
    org.openjsse.sun.security.ssl.TestHkdf
retval=$?
