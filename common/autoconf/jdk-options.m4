...
#    core: interpreter only, no compiler (only works on some platforms)
  ...
  # Allow to build client compiler on 64-bit platforms
  # TODO: remove later
  if test "x$JVM_VARIANT_CLIENT" = xNotrue; then
    if test "x$OPENJDK_TARGET_CPU_BITS" = xNo64; then
  ...
  if test "x$VAR_CPU" = xppc64 -o "x$VAR_CPU" = xppc64le -o "x$OPENJDK_TARGET_CPU" = xaarch32; then
    INCLUDE_SA=false
  fi
  if test "x$VAR_CPU" = xppc ; then
    INCLUDE_SA=false
  fi
  if test "x$VAR_CPU" = xppc ; then
    INCLUDE_SA=false
...
[
  ...
  if test "x$enable_jfr" = "xno"; then
    ENABLE_JFR=false
  elif test "x$enable_jfr" = "xauto"; then
...
[
  ...
  AC_ARG_WITH(abi-name, [AS_HELP_STRING([--with-abi-name],
      [Set the OS ABI name to be returned with sub.arch.abi system property @<:@toolchain dependent@:>@])])
  if test "x$with_abi_name" != x; then
    if test "x$OPENJDK_TARGET_CPU" != xaarch32; then
      AC_MSG_ERROR([--with-abi-name is only available on aarch32"])
    else
      JDK_ARCH_ABI_PROP_NAME="$with_abi_name";
    fi
  else
    if test "x$OPENJDK_TARGET_CPU" = xaarch32; then
      AC_MSG_ERROR([--with-abi-name must be supplied])
    fi
  fi
