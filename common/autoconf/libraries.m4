...
[
  ...
  ###############################################################################
  #
  # Check if iconv is in libc or libiconv
  #

  # Can't use AC_SEARCH_LIBS, iconv.h needs to be included in test

  AC_MSG_CHECKING([iconv link flags (for standalone libiconv)])

  save_LDFLAGS=$LDFLAGS
  LDFLAGS="$LDFLAGS -liconv"
  AC_LINK_IFELSE([AC_LANG_PROGRAM([#include <iconv.h>], [return (int)iconv;])],
      [LIBICONV="-liconv"],
      [LIBICONV=""])
